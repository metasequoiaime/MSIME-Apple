...
export function candidateTextStyle(
  value: unknown,
  numberValue?: unknown,
  accentValue?: unknown,
  ...
  return color || numberColor || accent || selected || hover || surface || border
    ? ({
        ...(color ? { "--cand-text": color } : {}),
        ...(numberColor ? { "--cand-num": numberColor } : {}),
        ...(accent ? { "--cand-accent": accent } : {}),
        ...(selected ? { "--cand-selected": selected } : {}),
        ...(hover ? { "--cand-hover": hover } : {}),
        ...(surface ? { "--cand-bg": surface } : {}),
        ...(border ? { "--cand-border": border } : {}),
      } as CSSProperties)
    : {};
}
