...
export function candidateFontSize(value: unknown): number {
  return typeof value === "number" && Number.isInteger(value) && value >= 12 && value <= 32 ? value : 18;
}
export function candidatePreeditFontSize(value: unknown): number {
  return typeof value === "number" && Number.isInteger(value) && value >= 12 && value <= 32 ? value : 15;
}
