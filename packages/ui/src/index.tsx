  ...
  return <div className="settings-shell" onPointerDownCapture={event => {
    ...
    {draft && page !== "typing-statistics" && page !== "account" && page !== "chat" && page !== "community" && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      <fieldset disabled={busy} hidden={page !== "appearance"} aria-label="外观">
        ...
        {showCandidateFontControls ? <CandidateFontControls value={draft} onChange={patch => setDraft({ ...draft, ...patch })} readFonts={client.listFontFamilies} windows={host?.platform === "windows"} englishFont={host?.platform === "windows" || host?.platform === "macos"} /> : <div className="section"><small>当前宿主的候选面板不支持自定义字体或字号。</small></div>}
