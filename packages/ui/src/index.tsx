...
import { asrProviderUpdate, polishProviderUpdate, ASR_PROVIDER_DEFAULTS, POLISH_PROVIDER_DEFAULTS } from "./voice-providers";
...
const defaultMixedInput: MixedInputPreferences = { english: true, minimum_prefix: 5, emoji: true, kaomoji: false };
  ...
  return <div className="settings-shell" onPointerDownCapture={event => {
    ...
    {draft && page !== "typing-statistics" && page !== "account" && page !== "chat" && page !== "community" && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      <fieldset disabled={busy} hidden={page !== "appearance"} aria-label="外观">
        <AppearanceCandidatePreview preferences={{ ...draft, candidate_english_font: host?.platform === "windows" ? draft.candidate_english_font ?? "Segoe UI" : undefined }} scan={client.scanSkinCatalog} readImage={client.readSkinImage} resolveFonts={client.resolveFontFamilies} active={page === "appearance"} revision={snapshot?.revision ?? 0} />
        ...
        {showCandidateFontControls ? <CandidateFontControls value={draft} onChange={patch => setDraft({ ...draft, ...patch })} readFonts={client.listFontFamilies} windows={host?.platform === "windows"} /> : <div className="section"><small>当前宿主的候选面板不支持自定义字体或字号。</small></div>}
      ...
      <fieldset disabled={busy} hidden={page !== "input"} aria-label="输入">
        ...
        <div className="section" role="group" aria-label="在线翻译服务">
          {linuxPlatform ? <>
            ...
            {windowsPlatform && tencentTranslation.enabled && credentialTestControl("translation.tencent", "测试腾讯云翻译配置", {
      ...
      <fieldset disabled={busy} hidden={page !== "about"} aria-label="关于">
        ...
        {!linuxPlatform && <button type="button" className="about-link-row about-document-link" onClick={() => void openExternalUrl(clientHostedPlatform ? androidPrivacyUrl : privacyUrl)}><span className="about-link-title">隐私政策</span><span aria-hidden="true">↗</span></button>}
        </div>
      ...
      <fieldset disabled={busy} hidden={page !== "voice"} aria-label="语音输入">
        {systemVoice ? <div className="section panel-launch-card"><div className="section-title">macOS 系统语音</div><p className="panel-inline-note">保存设置后，在目标应用中启用水杉输入法，使用下方语音快捷键录音。不需要识别 API Key；首次使用需授予麦克风和语音识别权限。服务可用性及是否联网由 macOS 决定，可选文本润色仍使用你配置的云服务。</p></div> : androidPlatform ? <div className="section panel-launch-card"><div className="section-title">Android 系统语音</div><p className="panel-inline-note">从键盘工具栏的“语音”入口调用设备上的系统语音识别服务。识别结果会回到键盘，确认后才插入当前输入框。</p></div> : <div className="section panel-launch-card"><div className="section-header panel-launch-row"><span className="section-title">打开语音输入<small>{linuxPlatform ? "录音和识别由已配置的 provider 服务完成" : "录音和识别在本机完成"}</small></span><button type="button" className="secondary panel-open-button" disabled={!client.openVoice} onClick={() => void openPanel(client.openVoice)}>打开</button></div>{linuxPlatform && <p className="panel-inline-note">没有 provider 时可继续使用 IBus 属性中的入口；服务负责录音、模型和凭据。</p>}</div>}
        ...
        {showVoiceCaptureDevices && <div className="section"><div className="section-title">录音设备<small>保存后从下一次录音生效，不打断当前录音</small></div>
          <label className="section-header"><span className="section-title">录音后端</span><select aria-label="录音后端" value={voiceInput.capture_backend ?? ""} onChange={event => updateVoice({ capture_backend: event.target.value as VoiceInputPreferences["capture_backend"], capture_device: "" })}><option value="">{windowsPlatform ? "系统默认" : "沿用服务设置"}</option><option value="auto">自动选择</option>{!windowsPlatform && <><option value="pulse">PulseAudio</option><option value="pipewire">PipeWire</option><option value="alsa">ALSA</option><option value="macos">CoreAudio</option></>}<option value="windows">Windows Audio</option>{windowsPlatform && voiceInput.capture_backend && !["auto", "windows"].includes(voiceInput.capture_backend) && <option value={voiceInput.capture_backend} disabled>{voiceInput.capture_backend}（此平台不可用）</option>}</select></label>
