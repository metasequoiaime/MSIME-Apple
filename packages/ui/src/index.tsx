  ...
  return <div className="settings-shell" onPointerDownCapture={event => {
    ...
    {draft && page !== "typing-statistics" && page !== "account" && page !== "chat" && page !== "community" && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      ...
      <fieldset disabled={busy} hidden={page !== "voice"} aria-label="语音输入">
        ...
        {!androidPlatform && !systemVoice && providerPresetControls("识别服务", ASR_PROVIDER_DEFAULTS[String(voiceInput.asr_provider)], voiceInput.asr_model ?? "", asr_model => updateVoice({ asr_model }))}
        ...
        {!androidPlatform && !systemVoice && <div className="section"><label className="section-header"><span className="section-title">Doubao 资源 ID<small>仅由 Doubao provider 使用</small></span><input aria-label="Doubao 资源 ID" value={voiceInput.asr_resource_id ?? ""} onChange={event => updateVoice({ asr_resource_id: event.target.value })} /></label></div>}
