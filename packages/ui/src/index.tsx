  ...
  return <div className="settings-shell">
    ...
    {draft && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      ...
      <fieldset disabled={busy} hidden={page !== "voice_input"} aria-label="语音输入">
        ...
        <div className="section"><label className="section-header"><span className="section-title">启用文本润色<small>ASR 转写后整理文本</small></span><input aria-label="启用文本润色" type="checkbox" checked={voice.polish_enabled} onChange={event => updateVoice({ polish_enabled: event.target.checked })} /></label><label>润色模型<input aria-label="润色模型" value={voice.polish_model} onChange={event => updateVoice({ polish_model: event.target.value })} /></label><label>润色 API Token<input aria-label="润色 API Token" type="password" value={voice.polish_token} onChange={event => updateVoice({ polish_token: event.target.value })} /></label><label>润色接口地址<input aria-label="润色接口地址" type="url" value={voice.polish_endpoint} onChange={event => updateVoice({ polish_endpoint: event.target.value })} /></label><textarea aria-label="润色提示词内容" value={voice.polish_prompt} onChange={event => updateVoice({ polish_prompt: event.target.value })} /></div>
