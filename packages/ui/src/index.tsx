...
export async function loadAllPersonalDictionaryEntries(
  ...
  for (const [kind] of personalDictionaryExportKinds) {
    ...
    while (hasMore && offset <= 1_000_000) {
      ...
      // An empty page ends the kind. Breaking out with `hasMore` still true made a kind with no entries read as one past the export limit, so export-all failed for anyone who had never added, say, a wubi word.
