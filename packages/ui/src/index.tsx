...
import { candidateFontSize, candidateFontSizes, candidatePreeditFontSize } from "./candidate/candidate-font-size";
...
import { asrProviderUpdate, polishProviderUpdate, ASR_PROVIDER_DEFAULTS, POLISH_PROVIDER_DEFAULTS } from "./voice/voice-providers";
import { POLISH_PRESET_IDS, POLISH_PRESET_NAMES, isPolishCustomSlot, normalizePolishSlot, polishPresetPrompt } from "./voice/polish-presets";
import { SkinToolbarPreview } from "./skin/skin-toolbar-preview";
import { ScreenKeyboardPreview, touchKeyboardSkinOptions } from "./keyboard/screen-keyboard-preview";
...
import { defaultTouchKeyboardSkinDesign, type AiSkinClient, type CustomSkinLibraryClient, type TouchKeyboardSkinDesign } from "./keyboard/touch-keyboard-skin-design";
export type { AiSkinClient, AiSkinProposal, AiSkinProgress, CustomSkinLibraryAction, CustomSkinLibraryClient, SavedTouchKeyboardSkin, TouchKeyboardSkinDesign } from "./keyboard/touch-keyboard-skin-design";
...
export { AccountPage, type AccountChallenge, type AccountClient, type AccountCommunityDestination, type AccountPreferenceSchema, type AccountPreferences, type AccountPreferenceValue, type AccountProfile, type AccountProviders, type AccountUser, type AppIconClient, type AppIconInfo, type SettingsSyncClient } from "./account/account-page";
export { ChatPage, type ChatClient, type ChatMessage, type ChatModel, type ChatModels } from "./chat/chat-page";
export { HomePage, type HomePageActions } from "./keyboard/home-page";
export { WelcomeFlowPage, type OnboardingActions, type OnboardingInputScheme } from "./account/onboarding-page";
export { SettingsStartupPage } from "./settings/settings-startup-page";
export { CommunitySkinsPage, type CommunitySkin, type CommunitySkinClient, type CommunitySkinDownload, type CommunitySkinPage, type CommunitySkinTrial } from "./community/community-skins";
...
export { POLISH_CUSTOM_IDS, POLISH_PRESETS, POLISH_PRESET_IDS, POLISH_PRESET_NAMES, isPolishCustomSlot, normalizePolishSlot, polishPresetPrompt, type PolishPresetId } from "./voice/polish-presets";
export { ASR_PROVIDER_DEFAULTS, POLISH_PROVIDER_DEFAULTS, asrProviderUpdate, polishProviderUpdate, type ProviderDefaults } from "./voice/voice-providers";
export { candidateTemplate, candidateThemeStylesheet, type CandidateAppearance, type CandidateOrientation, type CandidateTheme } from "./candidate/candidate-themes";
import { compareVersions, describeInstallerTrust, parseVersion, validateGitHubRelease, validateManifest, type GitHubRelease, type UpdateManifest, type ValidatedUpdate } from "./settings/update-manifest";
export { serializeWindowHostMessage, type WindowControl, type WindowHostMessage, type WindowResizeEdge } from "./keyboard/window-host";
...
export const AI_PROVIDER_OPTIONS: readonly {
  id: string; title: string; endpoint: string; model: string;
  models?: readonly string[]; documentation?: string;
}[] = [
  { id: "everyapi", title: "EveryAPI", endpoint: "https://api.everyapi.ai/v1/chat/completions", model: "deepseek-v4-flash", models: ["deepseek-v4-flash", "deepseek-v4-pro", "claude-sonnet-5", "glm-5.3-flash"], documentation: "https://everyapi.ai/models" },
  { id: "openai", title: "OpenAI", endpoint: "https://api.openai.com/v1/chat/completions", model: "gpt-4.1-mini", models: ["gpt-4.1-mini"], documentation: "https://developers.openai.com/api/docs/models/gpt-4.1-mini" },
  { id: "anthropic", title: "Anthropic · Claude", endpoint: "https://api.anthropic.com/v1/chat/completions", model: "claude-sonnet-4-6", models: ["claude-sonnet-4-6", "claude-opus-5"], documentation: "https://platform.claude.com/docs/en/cli-sdks-libraries/libraries/openai-sdk" },
  { id: "gemini", title: "Google · Gemini", endpoint: "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", model: "gemini-3.8-flash", models: ["gemini-3.8-flash", "gemini-2.5-flash"], documentation: "https://ai.google.dev/gemini-api/docs/openai" },
  { id: "deepseek", title: "DeepSeek", endpoint: "https://api.deepseek.com/chat/completions", model: "deepseek-v4-flash", models: ["deepseek-v4-flash", "deepseek-v4-pro"], documentation: "https://api-docs.deepseek.com/" },
  { id: "qwen", title: "通义千问 · 阿里云百炼", endpoint: "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions", model: "qwen-plus", models: ["qwen-plus"], documentation: "https://help.aliyun.com/zh/model-studio/compatibility-of-openai-with-dashscope" },
  { id: "kimi", title: "Kimi · 月之暗面", endpoint: "https://api.moonshot.cn/v1/chat/completions", model: "kimi-k2.6", models: ["kimi-k2.6", "kimi-k2.5"], documentation: "https://platform.kimi.com/docs/api/chat" },
  { id: "zhipu", title: "智谱 · GLM", endpoint: "https://open.bigmodel.cn/api/paas/v4/chat/completions", model: "glm-4.7", models: ["glm-4.7", "glm-4.7-flashx"], documentation: "https://docs.bigmodel.cn/cn/guide/models/text/glm-4.7" },
  { id: "siliconflow", title: "硅基流动", endpoint: "https://api.siliconflow.cn/v1/chat/completions", model: "Qwen/Qwen3.6-27B", models: ["Qwen/Qwen3.6-27B"], documentation: "https://docs.siliconflow.cn/docs/userguide/capabilities/text-generation" },
  { id: "groq", title: "Groq", endpoint: "https://api.groq.com/openai/v1/chat/completions", model: "llama-3.3-70b-versatile", models: ["llama-3.3-70b-versatile", "llama-3.1-8b-instant"], documentation: "https://console.groq.com/docs/quickstart" },
  { id: "openrouter", title: "OpenRouter", endpoint: "https://openrouter.ai/api/v1/chat/completions", model: "openrouter/auto", models: ["openrouter/auto"], documentation: "https://openrouter.ai/docs/quickstart" },
...
export function SettingsPage({ client, initialPage, onReplayOnboarding }: { client: SettingsClient; initialPage?: string; onReplayOnboarding?: () => void }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void client.loadMacosShuangpinKeymap().then(value => {
  ...
  const providerPresetControls = (
    ...
    return <div className={className}>
      {models.length > 0 && <label className="section-header"><span className="section-title">预置模型<small>服务商已知支持的模型；也可以在模型框中自行填写</small></span>
        <select aria-label={`${label}预置模型`} value={models.includes(model) ? model : ""} onChange={event => { if (event.target.value) onSelectModel(event.target.value); }}>
          <option value="">自定义模型…</option>
          {models.map(entry => <option key={entry} value={entry}>{entry}</option>)}
        </select></label>}
      {linkable && <button type="button" className="secondary" onClick={() => void openExternalUrl(documentation)}>{label}接入说明与 API Key</button>}
    </div>;
  };
  ...
  return <div className="settings-shell" onPointerDownCapture={event => {
    ...
    {client.typingStatistics && page === "typing-statistics" && <TypingStatisticsPage client={client.typingStatistics} mobile={mobilePlatform} platform={client.host?.platform} openSystemSettings={client.openSystemKeyboardSettings} />}
    {draft && page !== "typing-statistics" && page !== "account" && page !== "chat" && page !== "community" && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      ...
      <fieldset disabled={busy} hidden={page !== "input"} aria-label="输入">
        ...
        {macosPlatform && client.loadMacosShuangpinKeymap && macosShuangpinKeymap !== undefined && <div className="section" hidden={client.touchKeyboardSchemes || draft.scheme !== "shuangpin"}>
          <label className="section-header"><span className="section-title">输入时显示双拼键位提示<small>双拼输入时显示当前方案的键位图，完成上屏后自动隐藏。</small></span><input aria-label="输入时显示双拼键位提示" className="toggle" type="checkbox" checked={macosShuangpinKeymap} onChange={event => setMacosShuangpinKeymap(event.target.checked)} /></label>
        </div>}
        ...
        <div className="section" role="group" aria-label="在线翻译服务">
          {linuxPlatform ? <>
            ...
            <div className="input-option-divider" />
            <label className="section-header"><span className="section-title">SecretId</span><input aria-label="腾讯云 SecretId" type="text" autoComplete="off" spellCheck={false} value={tencentTranslation.secret_id} disabled={!candidateTranslations || !tencentTranslation.enabled} onChange={event => setDraft({ ...draft, tencent_tmt: { ...tencentTranslation, secret_id: event.target.value } })} placeholder="AKIDxxxxxxxxxxxxxxxx" /></label>
            <div className="input-option-divider" />
            ...
            <div className="input-option-divider" />
            <label className="section-header"><span className="section-title">地域</span><input aria-label="腾讯云地域" type="text" autoComplete="off" spellCheck={false} value={tencentTranslation.region} disabled={!candidateTranslations || !tencentTranslation.enabled} onChange={event => setDraft({ ...draft, tencent_tmt: { ...tencentTranslation, region: event.target.value } })} placeholder="ap-guangzhou" /></label>
            {(windowsPlatform || macosPlatform) && tencentTranslation.enabled && credentialTestControl("translation.tencent", "测试腾讯云翻译配置", {
              secret_id: tencentTranslation.secret_id, secret_key: tencentTranslation.secret_key, region: tencentTranslation.region,
            }, !candidateTranslations || Boolean(tencentCredentialIssue(tencentTranslation.secret_id, tencentTranslation.secret_key, tencentTranslation.region)))}
      ...
      <fieldset disabled={busy} hidden={page !== "about"} aria-label="关于">
        <div className="section document-hero about-hero"><div className="about-mark"><img src={logo} alt="水杉 IME" /></div><div><div className="document-eyebrow">Metasequoia IME</div><div className="document-hero-title">水杉 IME</div><p>{platformAboutDescription}</p></div></div>
      ...
      <fieldset disabled={busy} hidden={page !== "handwriting"} aria-label="手写识别板">
        {iosPlatform ? <div className="section panel-launch-card"><div className="section-title">iOS 键盘手写</div><p className="panel-inline-note">请在 iOS 系统键盘设置中启用水杉键盘，并在键盘内切换到“手写”输入方案。首次使用会按需下载中文识别模型；需要开启“允许完全访问”才能下载模型，下载后可离线识别。</p><p className="panel-inline-note">手写由键盘扩展在当前输入框内完成，不打开独立的 Tauri 手写面板；笔迹和识别结果不会上传，Google ML Kit 仅可能发送性能及使用统计。</p>{client.openSystemKeyboardSettings && <button type="button" className="secondary" onClick={() => void client.openSystemKeyboardSettings!()}>打开系统键盘设置</button>}</div> : androidPlatform ? <div className="section panel-launch-card"><div className="section-title">Android 键盘手写</div><p className="panel-inline-note">请在 Android 系统输入法设置中启用水杉键盘，再从键盘方案切换到“手写”。首次使用时按需下载 Google ML Kit 中文手写模型；模型就绪后可离线识别。</p><p className="panel-inline-note">手写识别在 Android 键盘进程内完成，候选确认后才提交到当前编辑器；笔迹和识别结果不会上传，Google ML Kit 仅可能发送性能及使用统计。</p>{client.openSystemKeyboardSettings && <button type="button" className="secondary" onClick={() => void client.openSystemKeyboardSettings!()}>打开系统输入法设置</button>}</div> : macosPlatform ? <div className="section panel-launch-card"><div className="section-title">macOS 手写识别板</div><p className="panel-inline-note">手写面板需要当前输入法进程提供 IMK 输入会话；请从输入法悬浮工具栏或输入法菜单打开，识别候选会直接回到当前输入上下文。</p></div> : <div className="section panel-launch-card">
      ...
      <fieldset disabled={busy} hidden={page !== "voice"} aria-label="语音输入">
        ...
        {localVoice && <div className="section"><label className="section-header"><span className="section-title">Whisper 模型文件<small>ggml 模型的绝对路径，例如 /Users/you/models/ggml-large-v3-turbo.bin</small></span><input aria-label="Whisper 模型文件" value={voiceInput.asr_model_path ?? ""} placeholder="/path/to/ggml-model.bin" onChange={event => updateVoice({ asr_model_path: event.target.value })} /></label></div>}
        {!androidPlatform && serviceVoice && providerPresetControls("识别服务", ASR_PROVIDER_DEFAULTS[String(voiceInput.asr_provider)], voiceInput.asr_model ?? "", asr_model => updateVoice({ asr_model }))}
        {!androidPlatform && serviceVoice && <div className="section"><label className="section-header"><span className="section-title">识别模型<small>由 provider 服务选择对应模型</small></span><input aria-label="识别模型" value={voiceInput.asr_model ?? ""} onChange={event => updateVoice({ asr_model: event.target.value })} /></label></div>}
        ...
        {!androidPlatform && serviceVoice && <div className="section"><label className="section-header"><span className="section-title">Doubao 资源 ID<small>仅由 Doubao provider 使用</small></span><input aria-label="Doubao 资源 ID" value={voiceInput.asr_resource_id ?? ""} onChange={event => updateVoice({ asr_resource_id: event.target.value })} /></label></div>}
        ...
        {!androidPlatform && <div className="section"><div className="section-title">文本润色 provider<small>识别结果可交给用户管理的服务润色</small></div>
          ...
          {providerPresetControls("文本润色", POLISH_PROVIDER_DEFAULTS[voiceInput.polish_provider ?? "siliconflow"], voiceInput.polish_model ?? "", polish_model => updateVoice({ polish_model }), "provider-preset-section")}
      ...
      <fieldset disabled={busy} hidden={page !== "ai"} aria-label="AI 辅助">
        ...
        {providerPresetControls("AI ", AI_PROVIDER_OPTIONS.find(option => option.id === ai.provider), ai.model, model => updateAi({ model }))}
