  ...
  return <div className="settings-shell">
    ...
    {draft && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      ...
      <fieldset disabled={busy} hidden={page !== "skin"} aria-label="皮肤">
        ...
        <div className="section help-section"><div className="section-title">内置皮肤</div><div className="skin-grid" role="list" aria-label="内置皮肤"><div className="skin-card"><span className="skin-card-title">Fluent</span>{selectedSkin === "fluent" && <small role="status">当前皮肤</small>}<HostActionButton label="应用 Fluent" success="已应用" action={client.skin?.select ? async () => { await client.skin!.select!("fluent"); setSelectedSkin("fluent"); } : undefined} /></div><div className="skin-card"><span className="skin-card-title">微信绿</span>{selectedSkin === "wechat" && <small role="status">当前皮肤</small>}<HostActionButton label="应用 微信绿" success="已应用" action={client.skin?.select ? async () => { await client.skin!.select!("wechat"); setSelectedSkin("wechat"); } : undefined} /></div><div className="skin-card"><span className="skin-card-title">Graphite</span>{selectedSkin === "graphite" && <small role="status">当前皮肤</small>}<HostActionButton label="应用 Graphite" success="已应用" action={client.skin?.select ? async () => { await client.skin!.select!("graphite"); setSelectedSkin("graphite"); } : undefined} /></div><div className="skin-card"><span className="skin-card-title">杨柳青</span>{selectedSkin === "willow_green" && <small role="status">当前皮肤</small>}<HostActionButton label="应用 杨柳青" success="已应用" action={client.skin?.select ? async () => { await client.skin!.select!("willow_green"); setSelectedSkin("willow_green"); } : undefined} /></div></div></div>
        <div className="section help-section"><div className="section-title">外部皮肤</div><p className="about-disclaimer">将包含 skin.toml 的皮肤文件夹复制到宿主皮肤目录，然后刷新皮肤。</p><div className="section-header"><span className="section-title">皮肤目录</span><span><HostActionButton label="打开目录" action={client.skin ? () => client.skin!.openDirectory() : undefined} /><HostActionButton label="刷新皮肤" action={client.skin ? async () => { await client.skin!.refresh(); if (client.skin!.list) setExternalSkins(await client.skin!.list()); } : undefined} /></span></div>{externalSkins.map(skin => <div className="section" key={skin.id}><strong>{skin.name}</strong>{selectedSkin === skin.id && <span role="status">当前皮肤</span>}<small>{[skin.id, skin.version && `v${skin.version}`, skin.author].filter(Boolean).join(" · ")}</small><p>{skin.compatible ? (skin.description || "外部皮肤") : "当前客户端不兼容此皮肤"}</p>{skin.compatible && <HostActionButton label="应用" success="已应用" action={client.skin?.select ? async () => { await client.skin!.select!(skin.id); setSelectedSkin(skin.id); } : undefined} />}</div>)}{client.skin && externalSkins.length === 0 && <p className="capability-status-inline">尚未扫描到外部皮肤</p>}{!client.skin && <p className="capability-status-inline">皮肤宿主尚未接入</p>}</div>
      </fieldset>
