...
export function AppearanceCandidatePreview({ preferences, scan, readImage, active = true, revision = 0 }: {
  ...
  return <section className="section" aria-label="候选窗口预览">
    ...
    {builtin ? <div className={`skin-card-preview appearance-candidate-preview skin-${skin}`} data-preview-theme={theme}
      data-font-size={candidateFontSize(preferences.candidate_font_size)} style={{ ...candidateFontStyle(preferences), ...candidateTextStyle(preferences.candidate_text_color, preferences.candidate_number_color, preferences.candidate_accent_color), ...candidateFamilyStyle(preferences) }} aria-hidden="true">
