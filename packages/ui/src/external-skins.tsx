...
function ExternalSkinCard({ skin, selected, layout, onSelect, readImage, readFont, readToolbarCss, revision, activeTheme }: {
  ...
  const theme = override ?? (skin.themes.includes(activeTheme) ? activeTheme : skin.themes[0] === "light" ? "light" : "dark");
...
export function ExternalSkins({ scan, openDirectory, readImage, readFont, readToolbarCss, selected, layout, onSelect, activeTheme = "dark" }: {
  scan?: () => Promise<SkinCatalog>; openDirectory?: () => Promise<void>; readImage?: SkinImageReader; readFont?: SkinFontReader; readToolbarCss?: ToolbarCssReader; selected: string; layout: string; onSelect: (id: string) => void; activeTheme?: "dark" | "light";
}) {
  ...
  return <section aria-label="外部皮肤" className="external-skins">
    ...
    <div className="skin-grid">{catalog?.packages.map(skin => <ExternalSkinCard key={skin.id} skin={skin} selected={selected} layout={layout} onSelect={onSelect} readImage={readImage} readFont={readFont} readToolbarCss={readToolbarCss} revision={revision} activeTheme={activeTheme} />)}</div>
