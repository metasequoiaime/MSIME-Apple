...
export async function prepareToolbarFonts(css: string, resolve: (relative: string) => Promise<ArrayBuffer>) {
  ...
  if (!Array.from(sheet.cssRules).some(rule => rule.type === CSSRule.FONT_FACE_RULE))
  ...
  for (const rule of Array.from(sheet.cssRules)) {
    ...
    for (const source of splitCssFontList(style.getPropertyValue("src"))) {
      ...
      try {
        ...
        await face.load();
        loaded.push(face); found = true; break;
  ...
  for (let index = sheet.cssRules.length - 1; index >= 0; index--) {
    if (sheet.cssRules[index].type === CSSRule.FONT_FACE_RULE) sheet.deleteRule(index);
  }
  ...
  return {
    ...
    install: () => {
      ...
      try { for (const face of loaded) { document.fonts.add(face); added.push(face); } }
      catch { for (const face of added) document.fonts.delete(face); throw new Error("font installation failed"); }
      return () => { for (const face of added) document.fonts.delete(face); };
    },
  };
}
