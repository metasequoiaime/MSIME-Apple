...
Windows 的编译门禁不需要 Windows 机器。装好 MinGW（`x86_64-w64-mingw32-g++`）后跑一次 `platforms/windows/build-cross.sh x64` 把 vcpkg 引导到清单基线，之后 `verify-local.sh` 会自动接管这条路径，把 host DLL、TSF DLL、Server 与全部原生测试链接一遍。没有这套环境时该阶段仍然跳过，但会把这行命令打出来——它曾经在每台机器上都只打印「skipped」，而背后的原生构建同时坏了六处。
