...
");
    ...
    fn a_page_holds_only_rows_of_the_requested_kind() {
        ...
        let mut taken = 0;
        for (is_wubi, key) in &rows {
            if dictionary_row_matches(*is_wubi, key, "w") && selector.accept() {
                taken += 1;
            }
        }
        assert_eq!(taken, 5);
...
/// With more than a page of pinyin words, selecting Wubi showed an empty list on
/// page 1 even though Wubi entries existed. Filtering here instead means a page
/// always holds `limit` rows of what the user actually asked for.
