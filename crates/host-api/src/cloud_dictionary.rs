...
pub fn validate_cloud_request(request: &CloudDictionaryRequest) -> Result<(), &'static str> {
    ...
    match request {
        ...
        CloudDictionaryRequest::Add { kind, .. }
        | CloudDictionaryRequest::Import { kind, .. }
        | CloudDictionaryRequest::Export { kind, .. } => {
            if valid_kind(kind) {
                Ok(())
            } else {
                Err("invalid cloud dictionary request")
        ...
        CloudDictionaryRequest::Update { kind, revision, .. }
        | CloudDictionaryRequest::Delete { kind, revision, .. } => {
            if valid_kind(kind) && *revision > 0 {
                Ok(())
            } else {
                Err("invalid cloud dictionary request")
            }
