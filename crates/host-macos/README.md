...
Permission checks never prompt. A key is rejected if posting is unavailable, the foreground belongs to this process, focus changes before posting, or event allocation fails. Startup focus restoration applies only when this shell is still foreground and the original process identity/launch time still matches; it does not send a pending key or steal focus from another application.
