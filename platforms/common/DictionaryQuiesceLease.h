...
// Raise the lease from inside an input host, for maintenance that host runs itself (the macOS input method's own dictionary window). It is written the way the settings window writes it: staged under `<lease>.<pid>` and renamed into place, so a reader never sees half of it, with an expiry at the bound. False when it could not be written.
inline bool raise_dictionary_quiesce_lease(const std::string &user_data,
                                           std::int64_t now_ms = dictionary_quiesce_now_ms()) {
  if (user_data.empty() || user_data.front() != '/') return false;
  ...
  staged += "." + std::to_string(getpid());
  {
    ...
    if (!(file << (now_ms + kDictionaryQuiesceLeaseMaxMs) << '\n')) {
...
inline void lower_dictionary_quiesce_lease(const std::string &user_data) {
  if (user_data.empty() || user_data.front() != '/') return;
  std::error_code ignored;
  std::filesystem::remove(std::filesystem::path(user_data) / std::string(kDictionaryQuiesceLeaseName), ignored);
}
