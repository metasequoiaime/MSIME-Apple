...
- (BOOL)handleEvent:(NSEvent *)event client:(id)sender {
    // The source times stage=handle/queue/reply-send across its IPC hop; the engine runs in this process, so one handle stage covers the same span. Only the event type and the outcome are logged, never the key.
...
// A key the input method does not consume is typed by the application itself and never reaches a commit path, so it is counted here, the way MSIME-Windows counts every uneaten OnTestKeyDown exit. Observation only: the routing decision above is returned unchanged. IMK delivers each event to handleEvent: once, so the source's per-event dedup is not needed.
...
- (BOOL)routeEvent:(NSEvent *)event client:(id)sender {
    CGEventRef nativeEvent = event.CGEvent;
