...
class BuildNumberTests(unittest.TestCase):
    ...
    def test_a_dispatch_with_a_tag_is_accepted(self):
        """手动发布和升版本这两条路都必须过得了第一步。

        它们曾经一起断掉:校验里留着 `test "$REQUESTED_PLATFORM" = both`,而 platform 这个输入在 workflow 拆成两个之后就没有了。空字符串对比 both 恒假,于是每一次带 tag 或带 bump_version 的手动触发都在「Validate invocation」直接 exit 1 —— 报错只有一行,看不出是在拦什么,而两条路平时都不走,红不了。
        """
    ...
    # 这里原先有一条 test_only_a_run_creating_its_own_draft_may_choose_its_platforms,用八组
    # (bump, tag, platform) 断言 platform 输入不能和 tag 的前缀矛盾。那个输入在发布拆成两个 workflow
    # 时就没有了,而它一直是绿的 —— 因为校验里同样残留的 `test "$REQUESTED_PLATFORM" = both` 把每一种
    # 组合都拒掉,于是每个「应当失败」的用例都如愿失败,每个「应当成功」的用例…也失败,只是没人看见,
    # 因为那三条恰好都不在断言里。删掉那条失效校验之后它立刻红了,这才是它第一次说真话。
    #
    # 平台现在由文件决定,由下面那条用例守着;手动触发能不能过第一步,由
    # test_a_dispatch_with_a_tag_is_accepted 守着。
