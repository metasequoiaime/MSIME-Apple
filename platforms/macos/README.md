...
输入法菜单的“候选预览…”打开独立的原生预览窗口，展示当前预览皮肤的输入效果、横排/竖排候选和悬浮状态栏示意，并支持预览明暗主题、布局、每页数量、字号及外部装饰图滚动查看。预览控件只使用独立预览偏好，不写共享配置、不调用输入会话；实际候选面板仍由共享设置快照驱动。

直接启动宿主 bundle 并传入 `--preferences` 可打开同一预览窗口；该路径使用普通 AppKit 激活策略，不启动 IMKServer。输入法服务启动时仍保持后台模式。

预览窗口中的“浏览所有皮肤…”打开卡片式皮肤目录：展示四套内置皮肤和扫描到的外部包，每张卡片提供独立明暗预览与单选启用开关，已启用项再次点击不会关闭。外部包通过“打开目录 → 复制皮肤文件夹 → 刷新皮肤”加载，并显示空状态和无效包诊断；目录操作只在用户点击时执行，失败不会替换现有配置。
...
候选可见时，控制器按布局消费对应的候选移动方向；页码超过一页时显示 Apple 风格的 `‹` / `›` 鼠标翻页按钮，首页/末页禁用越界方向。按钮点击通过共享运行时的上一页/下一页命令处理，候选 ID 和组合状态仍由共享层校验。测试覆盖分页按钮、页边界、横排实际宽度和可见/隐藏状态；Home/End 在候选可见时选择当前页首尾，隐藏时保留编辑光标移动；横排左右键和所有布局的上下键移动候选，竖排左右键只消费。可配置翻页快捷键尚待迁移。
...
从仓库根目录让设置页写入同一份隔离配置：`MSIME_CLIENT_STATE_DIR="$PWD/target/macos-state" pnpm --filter @msime/desktop tauri dev`。保存后活跃宿主通常在下一次轮询收到快照，候选布局和字号立即刷新，Engine 相关设置仍按当前组词结束后生效；无需 Tauri 常驻。此为代码和原生桥接层已验证的链路，尚未验证系统安装后的设置窗口到编辑器全程交互。

文本适配测试验证提交、ASCII 光标和清除预编辑；它不是系统输入源安装后在编辑器中的验收。系统级焦点、候选位置和键盘输入仍需真实宿主验证后才能标为完成。

候选导航修正对照 Apple develop 固定提交 `14a4b637e51c0da5a4ddff69328cee7ce7431e83`。ShortcutTest 覆盖横竖两种布局、四个方向与 Home/End 的可见/隐藏路由；原生 CMake 构建及 8 项 CTest 通过。尚未执行安装后真实编辑器验收。
