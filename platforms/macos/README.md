...
当前预览支持 ASCII 输入、退格、移动编辑光标、空格选择、回车原文、Esc 取消、候选上下移动和翻页、鼠标选词，以及由共享运行时处理的当前页数字选词。候选面板显示对应数字；Engine 优先接收数字输入，保留 Unicode 等输入模式。完整标点路由、设置热更新、菜单与正式安装尚待接入。原始 ASCII 编辑串用于内联预编辑，光标单位与 Engine 一致；日语等美化预编辑另行处理。
