...
原生菜单提供“简体输出”（默认）与“繁体输出”，保存到宿主偏好 `MSIMEClientTraditionalOutput`。调用 `msime-host-api` 的 `msime_client_simplified_to_traditional` 导出转换候选显示、完整 tooltip 和最终上屏文本，与 Windows、Linux 及来源同用 OpenCC s2t 词级词典（「头发」→「頭髮」而非逐字的「頭發」），输入不是合法 UTF-8 时保留原文；日语方案与 Unicode 精确码点模式不转换。以词定字（`[` `]`）从转换成繁体后的整条候选词里取首 / 末汉字，头发+`]` 上屏「髮」、皇后+`]` 上屏「后」，与候选窗显示的词一致，取出的字不再单独转换。转换只发生在原生展示/插入边界，Engine 原文、候选 ID、组合与运行时视图保持不变。

以词定字的高亮候选里没有汉字（英文单词、数字、emoji、颜文字）时，按键上屏候选词再接该键的标点，与 Windows 来源及 Linux 一致：GitHub+`[` 上屏「GitHub【」，`-` 按原样追加，`]`、`=` 走标点表；候选身份过期或 Engine 调用失败时按键被消费，不产出标点。
...
候选释义的位置随布局而定：竖排时释义以候选 0.78 倍字号显示在候选右侧的同一行，只占宽度不占高度，释义在输入后稍晚到达时候选窗不会变高；只有两种目标语言的双行释义比文字行更高时，行才随之增高，以免画进相邻一行；横排时释义在候选下方，所需高度事先预留，到达时同样不改变候选窗高度。
...
哪些中文候选会请求在线释义与来源的 `IsCloudTranslatableChinese` 相同：至少含一个汉字（含扩展区与兼容表意文字）且不含 emoji 或图形符号，ASCII 字母与数字不影响资格，所以 T恤、卡拉OK、3D打印这类混写词同样取回并记住在线释义；纯 ASCII 与含 emoji 的候选不上网。
...
中英、Emoji 和颜文字混输设置跟随 Windows 的 `mixed_input` 偏好迁移到 macOS。中英混输默认开启，触发阈值默认 5 个字符，Emoji 默认开启（与来源出厂的 `emoji_mixed_input = true` 一致）、颜文字混输默认关闭；阈值限制为 1–8，关闭中英混输时控件保留数值但暂时禁用。设置写入宿主偏好域，活动组合期间由共享 host-api 延迟应用，平台不复制 Engine 的候选混排算法；原生测试覆盖共享读取、持久化、控件联动和非法值保护。混输时英文候选默认排在首个中文候选之后；置顶或经候选学习升到第一位的英文词会成为第一个候选（云 / AI 候选到达后也保持），空格上屏的就是它，用户固定了位置的英文词停在其固定位置，云 / AI 候选到达后也不会被挤到后面。日文罗马字输入单个完整假名（如 `ka`）时，平假名与片假名（か / カ）占前两位，云候选排在它们之后。
...
Ctrl+退格按分词单位删读音，Ctrl+← / Ctrl+→ 按分词单位移光标，只认裸 Ctrl。组一个分几次选完的词时，Ctrl+退格把剩余读音删空后，已选的那一段仍留在标记文本里：退格把最后一次选词吃掉的读音还回来，再按 Ctrl+退格删掉最后一段已选的词，回车上屏，Esc 丢弃。没有可退回的选词时，普通退格删空读音仍把已选的词上屏。按键路由用 `MSIMEViewHasComposition` 判断组字，编辑串、候选或 `phrase_prefix` 任一非空都算，所以只剩已选词时回车、空格、Esc、退格与标点仍交给会话，不会漏给应用。
