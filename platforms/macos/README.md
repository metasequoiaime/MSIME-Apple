...
候选可见时，控制器按布局消费对应的候选移动方向；页码超过一页时显示 Apple 风格的 `‹` / `›` 鼠标翻页按钮，首页/末页禁用越界方向。按钮点击通过共享运行时的上一页/下一页命令处理，候选 ID 和组合状态仍由共享层校验。测试覆盖分页按钮、页边界、横排实际宽度和可见/隐藏状态；Home/End 页内导航和可配置翻页快捷键尚待迁移。
