...
SwiftUI 设置同步现覆盖 18 个当前宿主偏好：候选皮肤、布局、字号、页大小、输入方案、翻页快捷键，以及纠错、辅助码、中文标点、成对标点、标点锁定、中英文模式、切换快捷键、全角、浮动工具栏、繁体输出、五笔自动上屏、双拼提示。云端字段来自 Apple 固定提交 `2b0250f4dd7012520392b310dfcc0288c3208a75`，本地键与默认值以共享客户端实际宿主为准。布尔字段只接收 JSON 布尔值，标点锁定只接收 `follow`、`chinese`、`english`。旧云端快照缺少新增字段时不会部分应用，可先上传本机完整快照；其他平台云端字段由既有合并逻辑保留。双拼配置、辅助码方案、学习策略等其余设置仍需迁移；本地测试不是实际云端和已安装输入源验收。
