...
“账户状态…”菜单直接打开随 bundle 加载的 SwiftUI 账户窗口，不再要求手填账户 ID。登录、设置同步、云词典、剪贴板、快照及社区资源由该窗口的共享账户会话管理。此变更不迁移旧 Keychain 条目，也不改变独立“云剪贴板…”菜单的旧入口。`backend-account-entry` 测试验证原生到窗口的分派及接口缺失处理，`backend-library-load` 验证实际 dylib 的账户类与选择器；这些不替代已安装输入源的真实登录验收。
