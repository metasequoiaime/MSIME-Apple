...
trap 'rm -f "$out"' EXIT
sources=()
for source in "$root"/shared/backend/account/*.swift "$root"/shared/backend/clients/*.swift "$root"/shared/backend/content/*.swift "$root"/shared/backend/storage/*.swift "$root"/shared/backend-ui/*.swift "$root"/platforms/macos/src/backend/Backend*.swift; do
