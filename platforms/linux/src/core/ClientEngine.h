...
// Exit status of msime-client-ibus after it found its own program replaced by an upgrade and quit so the new build can take over. The launcher restarts it at once with --recovered, without the crash backoff; keep the value in step with upgraded_status in that script.
