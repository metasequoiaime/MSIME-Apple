...
语音波形展示通过 `WaveOverlaySurface` 注入宿主。X11 且同时提供 `x11`、`xfixes` 开发模块时，宿主使用不抢焦点、输入区域透明的原生浮层；没有可用 `DISPLAY`、X11 连接失败或处于 Wayland 会话时回退到 IBus 辅助栏。可用 `MSIME_WAVE_OVERLAY_BACKEND=ibus` 强制辅助栏，或用 `x11` 请求 X11 后端；Wayland layer-shell 后端仍待后续切片接入，不会伪造 Wayland 定位能力。
