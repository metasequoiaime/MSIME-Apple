...
Windows 的 `clipboard_history` 依赖独立剪贴板监听器和候选历史 UI；IBus Engine API 不提供剪贴板事件、读取或历史面板。Linux IBus 宿主不会读取或记录剪贴板内容，即使共享设置开启也保持禁用，避免把输入法进程扩展成无提示的剪贴板监视器。Linux 剪贴板历史若需实现，应由独立、明确授权的桌面服务承载。
