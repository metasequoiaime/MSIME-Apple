// Stands in for ibus-daemon on a plain dbus-daemon so a test can run the real msime-linux-ibus up to and past component registration without IBus, a desktop session or dictionaries.
