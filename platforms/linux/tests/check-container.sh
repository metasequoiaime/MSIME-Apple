...
if [[ ${2:-} == --fcitx5 ]]; then
  ...
  docker build -t "$test_image" -f "$repo_root/platforms/linux/tests/Dockerfile.fcitx5" "$repo_root/platforms/linux/tests"
fi
docker run --rm --init \
  ...
  "$test_image" bash platforms/linux/tests/in-container.sh
