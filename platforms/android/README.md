...
手写迁移已建立 Android 原生画布和平台识别器注入边界：画布限制 64 笔、每笔 512 个采样点，支持单笔撤销、清空、坐标夹取和尺寸变化失效；识别请求复制不可变笔画快照，并以 revision 拒绝过期结果，候选去重后最多 12 项。模型下载、ML Kit 识别实现、方案持久化和键盘内候选 UI 仍待后续切片。
