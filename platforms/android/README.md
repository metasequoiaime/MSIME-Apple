...
Android 账号设置中的云词典现通过同一认证会话访问 HTTPS API，内联提供四类词库的查询、分页、添加、编辑、删除、UTF-8 TSV/汉字自动注音导入和标准/Windows 导出；请求、响应、64 位小写 hex ID、权重、格式、文本和页长由共享 Rust/host 边界验证。导出文件只在用户点击导出时取得，不自动把云词库合并到本机；本切片尚未迁移 Apple 的云候选排序、完整目录编辑和全量快照替换页面，后续切片继续完成这些功能。账号会话和云词库内容不写入日志，真实账号、文件分享和设备生命周期仍需 Android 原生产品验收。
