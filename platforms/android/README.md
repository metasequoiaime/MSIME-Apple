...
Android 账号页的云剪贴板入口打开同包 Tauri 的共享云剪贴板面板，使用 Android 账号会话访问 HTTPS 云端接口；不自动读取系统剪贴板，只有用户明确在面板中添加文本时才上传。列表搜索、文本、64 位小写 hex ID、更新时间和分页均由共享 Rust/host 与 Android transport 做边界校验，单页最多 50 条。面板复用公共启用开关、搜索、添加、删除和复制流程，复制通过 Android `ClipboardManager` 写入系统剪贴板，不尝试桌面输入目标注入。关闭云剪贴板会删除云端历史；账号会话、云端内容和系统剪贴板均不写入日志。真实账号、网络失败和设备生命周期仍需 Android 原生产品验收。
