...
int main() {
  ...
  const std::string response =
      R"({"ok":true,"value":{"handled":true,"has_commit":true,"commit":"你","diagnostic":"","view":{"preedit":"ni","editing_text":"ni","candidates":[{"id":{"generation":7,"index":0},"text":"你"}]}}})";
  if (!EngineSessionAdapter::parse_result(response, &result, &error) ||
      !result.handled || !result.has_commit || result.commit != "你" ||
      result.view.generation != 7 || result.view.candidates.size() != 1 ||
