...
void FloatingToolbarWindow::refresh(bool enabled) {
  ...
  try {
    ...
    work = info.rcWork;
    const int width = scale(window_, kWidth), height = scale(window_, kHeight);
    const int margin = scale(window_, 20);
...
void FloatingToolbarWindow::paint() {
  ...
  if (value && shown_ && same(value->lease, shown_->lease)) {
    ...
    for (int i = 0; i < 4; ++i) { RECT cell{scale(window_, 8 + i * 72), scale(window_, 8), scale(window_, 72 + i * 72), scale(window_, 44)};
