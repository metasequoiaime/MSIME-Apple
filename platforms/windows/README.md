...
预览入口现已创建主线程 CandidateWindow，只读显示预编辑、当前页序号与高亮；通过 NOACTIVATE/TOOLWINDOW 样式及 WM_MOUSEACTIVATE 拒绝鼠标激活，不抢输入焦点。窗口使用系统颜色和 GUI 字体，长文本省略，位置限制到最近显示器工作区。主线程以最长 50ms 空闲等待轮询状态，消息批次有界，快照身份未变时不重复触发绘制；每次 WM_PAINT 重新读取候选，失焦/UILess/断开/停机隐藏，绘制错误关闭预览。当前未实现鼠标选词、工具栏、逐显示器 DPI 字体缩放、无障碍与 TSF 实机定位验收；使用系统默认 DPI 虚拟化，不能视为产品级窗口完成。原生窗口回归已交叉编译，尚未在 Windows 运行。
