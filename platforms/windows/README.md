...
窗口读取候选时尝试获取焦点锁和候选缓冲锁；忙碌即返回空值，暂时隐藏并在下一次刷新重新读取，不复用旧焦点内容、不等待持有焦点锁的慢管道写入。连接校验保留在焦点锁内，避免释放后撞上下一次发送；这不是整个 UI 无锁或实时延迟保证，仍需 Windows 实测。

控制器现在把当前 lease 的 HideCandidateWnd / ShowCandidateWnd / MoveCandidateWnd 更新到候选缓冲：隐藏输出无文本的不可见快照；移动只更新坐标、不解除隐藏；显示恢复已有可显示快照并更新锚点，但不覆盖 UILess/组合结束的不可见状态。下一个已确认键回复恢复其自身展示状态。事件只影响展示，不改写 Engine 组合、不从 TSF 包重放输入；过期/其他连接不能移动或显示当前候选。此补齐修复了无新按键时窗口忽略宿主移动/显隐的问题，原生生命周期语义仍需与实际 TSF 一并验收。
