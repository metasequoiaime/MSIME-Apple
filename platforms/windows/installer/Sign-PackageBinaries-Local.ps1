# 用本机自签名测试证书给安装文件（server_exe\ 与 tsf_dll\）里的 EXE/DLL 签名。
# 每台机器各自生成证书，本仓库不包含任何预置证书或指纹。
#
# 直接运行，不需要参数：
#   pwsh -File .\Sign-PackageBinaries-Local.ps1
#
# 证书在第一次运行时自动创建，并装进本机受信任存储（这一步需要管理员 PowerShell；
# 没有管理员权限只会警告，签名照做，只是 signtool verify 的链校验过不了）。
# 之后每次运行都会复用同一张证书，已经被它签过的文件也会自动跳过。
#
...
# 最外层的 @() 不能省：只有一个待签文件时，管道结果会是标量，.Count 在严格模式下会报错。
$targets = @(
    @(
        foreach ($directory in $packageDirectories) {
...
if ($targets.Count -eq 0) {
    throw '没有找到待签名的 EXE 或 DLL。'
}

Write-Warning '使用本机测试证书签名，产物只能内部测试，不要对外分发。'
