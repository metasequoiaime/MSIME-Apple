...
begin
  ...
  { 设置窗口是独立的 WinUI 3 msime-client-settings.exe；表情 / 手写 / 屏幕键盘面板仍由同目录的 Tauri MSIME Client Preview.exe 承载。两个进程都不在 Server 的进程树里，覆盖安装和卸载前由安装器统一停止。}
  StopProcess('{#MySettingsExeName}');
  StopProcess('MSIME Client Preview.exe');
end;
