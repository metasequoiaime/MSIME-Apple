...
The Engine revision comes from `engine-lock.json`. `Collect-Notices.ps1` reads
only the prepared Engine tree whose `.msime-engine-lock` marker matches that
commit; it never follows a gitlink or `.gitmodules` file. Each dependency prefix must provide at least
...
collect nested third-party archive licenses automatically, or grant permissions
missing from upstream. The Engine helpcode notice explicitly records unresolved
redistribution permissions, and its original text is retained. Supply/review
the nested archive, Rust/frontend, model and distribution-specific material;
