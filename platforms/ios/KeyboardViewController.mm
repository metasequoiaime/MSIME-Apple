...
- (void)actionPressed:(UIButton *)button {
    ...
    } else if ([button.currentTitle isEqualToString:@"首"]) [self apply:[self.session command:MSIME_MOVE_HOME error:nil]];
    ...
    else if ([button.currentTitle isEqualToString:@"上词"]) [self apply:[self.session command:MSIME_PREVIOUS_CANDIDATE error:nil]];
    else if ([button.currentTitle isEqualToString:@"下词"]) [self apply:[self.session command:MSIME_NEXT_CANDIDATE error:nil]];
