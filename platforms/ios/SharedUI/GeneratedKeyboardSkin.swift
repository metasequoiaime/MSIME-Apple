...
enum GeneratedKeyboardSkin {
  ...
  static func parse(_ response: String) throws -> SavedKeyboardSkin {
    ...
    // 按 Unicode 类别判断,不要把 CharacterSet.contains 当方法引用传进 contains(where:)。
    //
    // On iOS 26 that unapplied reference answers true for ordinary CJK: every scalar of 青瓷庭院
    // tests false on its own, and the same predicate handed to contains(where:) says the string
    // holds a control character. Every AI design with a Chinese name was rejected as malformed.
