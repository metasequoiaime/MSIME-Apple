...
test("compatible external skin can be applied through the host", async () => {
  ...
  await waitFor(() => expect(select).toHaveBeenCalledWith("ocean"));
...
test("voice ASR configuration persists", async () => {
  const client: SettingsClient = { load: vi.fn().mockResolvedValue(initial), save: vi.fn().mockImplementation(async (_revision, preferences) => ({ ...initial, revision: 8, preferences })) };
  render(<SettingsPage client={client} />);
  fireEvent.click(screen.getByRole("button", { name: "语音输入" }));
  await screen.findByRole("textbox", { name: "ASR 模型" });
  fireEvent.change(screen.getByRole("textbox", { name: "ASR 模型" }), { target: { value: "whisper-1" } });
  fireEvent.change(screen.getByLabelText("ASR Access Token / API Key"), { target: { value: "asr-token" } });
  fireEvent.click(screen.getByRole("button", { name: "保存设置" }));
  await screen.findByText("设置已保存。");
  expect(client.save).toHaveBeenCalledWith(7, expect.objectContaining({ voice_input: expect.objectContaining({ asr_model: "whisper-1", asr_token: "asr-token" }) }));
});

test("voice text polishing settings persist", async () => {
  const client: SettingsClient = { load: vi.fn().mockResolvedValue(initial), save: vi.fn().mockImplementation(async (_revision, preferences) => ({ ...initial, revision: 8, preferences })) };
  render(<SettingsPage client={client} />);
  fireEvent.click(screen.getByRole("button", { name: "语音输入" }));
  fireEvent.click(await screen.findByRole("checkbox", { name: "启用文本润色" }));
  fireEvent.change(screen.getByRole("textbox", { name: "润色模型" }), { target: { value: "gpt-4o-mini" } });
  fireEvent.click(screen.getByRole("button", { name: "保存设置" }));
  await screen.findByText("设置已保存。");
...
test("voice polishing provider selection persists", async () => {
  const client: SettingsClient = { load: vi.fn().mockResolvedValue(initial), save: vi.fn().mockImplementation(async (_revision, preferences) => ({ ...initial, revision: 8, preferences })) };
  render(<SettingsPage client={client} />);
  fireEvent.click(screen.getByRole("button", { name: "语音输入" }));
  await screen.findByRole("button", { name: "润色服务提供商" });
  fireEvent.click(screen.getByRole("button", { name: "润色服务提供商" }));
...
test("Doubao voice recognition options persist", async () => {
  const client: SettingsClient = { load: vi.fn().mockResolvedValue(initial), save: vi.fn().mockImplementation(async (_revision, preferences) => ({ ...initial, revision: 8, preferences })) };
  render(<SettingsPage client={client} />);
  fireEvent.click(screen.getByRole("button", { name: "语音输入" }));
  fireEvent.click(await screen.findByRole("checkbox", { name: "豆包语义顺滑" }));
  fireEvent.change(screen.getByRole("textbox", { name: "豆包热词表 ID" }), { target: { value: "table-1" } });
  fireEvent.click(screen.getByRole("button", { name: "保存设置" }));
  await screen.findByText("设置已保存。");
  expect(client.save).toHaveBeenCalledWith(7, expect.objectContaining({ voice_input: expect.objectContaining({ doubao_enable_ddc: true, doubao_boosting_table_id: "table-1" }) }));
});
...
test("feedback page provides issue and community links", async () => {
  render(<SettingsPage client={{ load: vi.fn().mockResolvedValue(initial), save: vi.fn() }} />);
  fireEvent.click(screen.getByRole("button", { name: "反馈" }));
...
test("handwriting page opens host capability when provided", async () => {
  const open = vi.fn().mockResolvedValue(undefined);
  render(<SettingsPage client={{ load: vi.fn().mockResolvedValue(initial), save: vi.fn(), handwriting: { open } }} />);
  fireEvent.click(screen.getByRole("button", { name: "手写识别" }));
  fireEvent.click(await screen.findByRole("button", { name: "打开" }));
  expect(open).toHaveBeenCalledTimes(1);
});
