...
export function DesktopCloudDictionary({ client }: { client: CloudDictionaryPanelClient }) {
  const [page, setPage] = useState<"dictionary" | "catalog" | "candidates" | "files" | "apply">(
    "dictionary",
  );
  if (page === "catalog")
    return (
      <CloudDictionaryCatalogPanel
        client={{ ...client, back: async () => setPage("dictionary") }}
      />
    );
  if (page === "candidates")
    return <CloudCandidatesPanel client={{ ...client, back: async () => setPage("dictionary") }} />;
  if (page === "files")
    return (
      <CloudDictionaryFilesPanel client={{ ...client, back: async () => setPage("dictionary") }} />
    );
  if (page === "apply")
    return (
      <CloudDictionaryApplyPanel client={{ ...client, back: async () => setPage("dictionary") }} />
    );
  return (
    <CloudDictionaryPanel
      client={{
