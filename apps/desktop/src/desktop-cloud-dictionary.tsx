...
import { CloudCandidatesPanel, CloudDictionaryCatalogPanel, CloudDictionaryPanel, type CloudDictionaryPanelClient } from "@msime/ui";
...
export function DesktopCloudDictionary({ client }: { client: CloudDictionaryPanelClient }) {
  const [page, setPage] = useState<"dictionary" | "catalog" | "candidates">("dictionary");
  ...
  return <CloudDictionaryPanel client={{ ...client, openCatalog: async () => setPage("catalog"), openCandidates: async () => setPage("candidates") }} />;
}
