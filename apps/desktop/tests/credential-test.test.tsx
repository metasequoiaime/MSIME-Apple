...
const snapshot: Snapshot = {
  ...
  preferences: {
    ...
    custom_translation: { enabled: false, endpoint: "https://fixture.invalid/translate", api_key: "synthetic-custom" },
    tencent_tmt: { enabled: false, secret_id: "", secret_key: "", region: "ap-guangzhou" },
    voice_input: {
      enabled: true, language: "zh-cn", asr_provider: "openai", asr_model: "whisper-1",
      asr_token: "must-not-cross-linux-provider-boundary", polish_enabled: true, polish_text: true,
      polish_provider: "deepseek", polish_model: "deepseek-v4-flash",
      polish_token: "must-not-cross-linux-provider-boundary",
    },
    ai_assistant: {
      enabled: true, provider: "deepseek", endpoint: "https://api.deepseek.com/chat/completions",
      model: "deepseek-v4-flash", candidate_limit: 3, token: "must-not-cross-linux-provider-boundary",
...
function mount(testApiCredential: NonNullable<SettingsClient["testApiCredential"]>) {
  render(<SettingsPage client={{
    load: async () => snapshot, save: vi.fn(), testApiCredential,
    host: { platform: "linux" } as never,
  }} />);
}

test("Linux settings tests translation configuration through its provider", async () => {
  const testApiCredential = vi.fn().mockResolvedValue({ ok: true, message: "连接成功，当前配置有效。" });
...
test("Linux voice and AI tests never send private tokens from preferences", async () => {
  ...
  expect(JSON.stringify(testApiCredential.mock.calls)).not.toContain("must-not-cross-linux-provider-boundary");
});
