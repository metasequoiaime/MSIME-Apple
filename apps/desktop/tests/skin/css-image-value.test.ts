...
test("rewrites quoted and unquoted package images, preserving non-resource content", async () => {
  const resolve = vi.fn().mockResolvedValue(data);
  expect(await rewriteCssImages("url('./images/a.png') center, url(images/b.png)", resolve)).toBe(
    `url("${data}") center, url("${data}")`,
  );
...
test.each([
  ...
  "C:/a.png",
  "%2e%2e/a.png",
])("rejects non-package URL %s without reading", async (url) => {
  const resolve = vi.fn();
...
test("allows bounded image data only, and rejects failed or invalid resolver output", async () => {
  ...
  expect(
    await rewriteCssImages("url(a.png)", vi.fn().mockRejectedValue(new Error("synthetic"))),
  ).toBeNull();
  expect(
    await rewriteCssImages("url(a.png)", vi.fn().mockResolvedValue('x"); body {display:none}')),
  ).toBeNull();
