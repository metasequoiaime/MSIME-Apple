...
import {
  customPropertyNames,
  decodeCustomPropertyName,
  maskVariableNames,
} from "../../../../packages/ui/src/skin/css-custom-property";
import { parseAnimationVariable } from "../../../../packages/ui/src/skin/skin-animation-variables";
import {
  hasUnresolvedCssResource,
  rewriteCssImages,
} from "../../../../packages/ui/src/skin/css-image-value";
...
test.each(["--", "name", "--a b", "--a\\", "--a\\\n", "--a)", "--a/*x*/b"])(
  "rejects invalid property tokens %s",
  (raw) => {
...
test("reserves escaped names and masks names without hiding fallback resources", async () => {
  expect(customPropertyNames("color:var(--\\74 est-var-0); --动画:pulse")).toEqual([
    "--test-var-0",
    "--动画",
  ]);
  expect(maskVariableNames("var(--a\\,b, url(remote.png))")).toBe(
    "var(--validated, url(remote.png))",
  );
  ...
  expect(
    await rewriteCssImages("var(--\\61, url(https://invalid.example/a.png))", resolve),
  ).toBeNull();
