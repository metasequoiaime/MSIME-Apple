...
test("maps local animation names and keeps list positions", () => {
  expect(rewriteAnimationNames("pulse, none, spin", names)).toEqual({
    value: "private-pulse, none, private-spin",
    partial: false,
  });
});
test.each(["var(--motion)", "pulse,", "pulse, ", "pulse()", "inherit", "revert", "missing", ""])(
  "disables unresolved animation names: %s",
  (value) => {
...
test("does not bind unknown names to another stylesheet", () => {
  expect(rewriteAnimationNames("pulse, foreign, spin", names)).toEqual({
    value: "private-pulse, none, private-spin",
    partial: true,
  });
});
