...
test.each(["windows", "macos"])("%s chat tests use the edited configuration and origin-bound token", async platform => {
  const snapshot: Snapshot = { format_version: 1, revision: 1, preferences: {
    ...
    voice_input: { enabled: true, language: "zh-cn", polish_enabled: true,
      polish_provider: "openai", polish_model: "fixture-model", polish_token: "synthetic-polish" },
    ai_assistant: { enabled: true, provider: "openai", endpoint: "https://fixture.invalid/chat",
      model: "fixture-model", candidate_limit: 3, tokens: { "https://fixture.invalid:443": "synthetic-ai" },
  ...
  expect(probe).toHaveBeenLastCalledWith("ai.assistant", {
    provider: "openai", endpoint: "https://fixture.invalid/chat", model: "edited-model", token: "synthetic-ai",
  });
  fireEvent.change(screen.getByLabelText("AI 接口地址"), { target: { value: "https://other.invalid/chat" } });
  expect((screen.getByRole("button", { name: "测试 AI 辅助配置" }) as HTMLButtonElement).disabled).toBe(true);
  ...
  expect(probe).toHaveBeenLastCalledWith("voice.polish", {
    provider: "openai", endpoint: "https://api.openai.com/v1/chat/completions", model: "fixture-model", token: "synthetic-polish",
  });
});
