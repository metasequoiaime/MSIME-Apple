...
test("font names preserve Unicode and enforce UTF-8 and list bounds", () => {
  ...
  for (const value of ["", "字".repeat(43), "a".repeat(129), undefined, null])
    expect(validFontFamily(value)).toBe(false);
...
test("font families are quoted literals, preserving fallback order", () => {
  ...
  expect(
    candidateFamilyStyle({
      candidate_font_family: "主字体",
      candidate_fallback_fonts: ["示例一", "示例二"],
    }),
  ).toEqual({
