...
test("the built-in preview consumes resolved CSS names", async () => {
  const preferences = {
    scheme: "quanpin" as const,
    shuangpin_profile: "xiaohe" as const,
    candidate_page_size: 6,
    learning: true,
    chinese_punctuation: true,
    candidate_font_family: "Synthetic W03",
  ...
  await waitFor(() =>
    expect(
      (
        view.container.querySelector(".appearance-candidate-preview") as HTMLElement
      ).style.getPropertyValue("--appearance-font-family"),
    ).toBe('"Synthetic Family", sans-serif'),
  );
...
test("aliases affect presentation without changing preferences or fallback order", async () => {
  const preferences = {
    candidate_font_family: "Synthetic W03",
    candidate_fallback_fonts: ["Fallback W04", "Synthetic W03"],
  };
...
test("changing the request discards stale aliases immediately and ignores late replies", async () => {
  ...
  const { result, rerender } = renderHook(
    ({ name }) =>
      useResolvedCandidateFonts(
        { candidate_font_family: name, candidate_fallback_fonts: [] },
        resolve,
      ),
...
test.each(
  [[], [""], ["too", "many"], ["bad\nname"], ["x".repeat(129)], null].map((reply) => ({ reply })),
)("invalid replies retain original names: $reply", async ({ reply }) => {
...
test("failed or unavailable resolution preserves the manual font", async () => {
  ...
  const { result, rerender } = renderHook(
    ({ resolver }) => useResolvedCandidateFonts(preferences, resolver),
    { initialProps: { resolver: resolve as ((names: string[]) => Promise<string[]>) | undefined } },
  );
  await act(async () => {});
  expect(result.current).toBe(preferences);
