...
test.each([false, true])(
  "old recognition completion does not end a restarted recording (reject=%s)",
  async (rejectOld) => {
    const pending: {
    ...
    render(
      <VoicePanel
        client={{ close: async () => {}, recognizeVoice, cancelVoice, sendText: vi.fn() }}
      />,
    );
    fireEvent.click(screen.getByRole("button", { name: "开始录音" }));
...
test("voice updates are ignored before recording and after cancellation", async () => {
  ...
  const onVoiceUpdate = vi.fn(async (listener) => {
    update = listener;
    return vi.fn();
  });
  ...
  render(
    <VoicePanel
      client={{ close: async () => {}, recognizeVoice, cancelVoice: async () => {}, onVoiceUpdate }}
    />,
  );
