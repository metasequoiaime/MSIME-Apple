...
function singleFile(): Plugin {
  return {
    ...
    enforce: "post",
    closeBundle() {
      const directory = join(import.meta.dirname, "../../platforms/harmony/entry/src/main/resources/rawfile/settings");
      ...
      for (const name of readdirSync(directory)) {
        ...
        const body = readFileSync(join(directory, name), "utf8").replace(/<\/(script|style)/gi, "<\\/$1");
        if (name.endsWith(".js")) {
          ...
          html = html.replace(new RegExp(`<link[^>]*href="[^"]*${name}"[^>]*>`), () => `<style>${body}</style>`);
        }
