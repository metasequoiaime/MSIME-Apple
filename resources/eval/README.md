...
### `sentences-harvested-v1.tsv` — 1105 条整句，从语料收割

用下面那套流程从 C4 中文语料收割、机器初筛、人工过目而来，每一条都是引擎自己会答错的句子。**它和手写集测的不是一回事**：手写集是正常句子的混合，看的是整体水平；这一份按构造全是难题，看的是重排器之间谁更强。两份的 top-1 不可互相比较，也不该合并。
