...
| 屏幕键盘 | 来源设置 `screenkb-settings.ts` | `main.cpp` → Tauri keyboard route、`desktop-keyboard.tsx`、Windows `send_key` 分支；macOS Tauri 路径现在在非激活面板挂载时捕获外部 PID+启动时间，并在每次按键前重新校验身份 | macOS Tauri 键盘不再把当前设置宿主误当成输入目标；布局、修饰键按下/释放、自动重复、焦点恢复及 DPI 仍需逐项核对。 |
