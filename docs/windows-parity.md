...
- **混输候选的座位表**：来源 `candidate_selection_policy.h` 的四种排布已实现进运行时的 `normalize_online_slots`，只在快照里确实存在云 / AI 候选时生效。插入是压缩的而非固定编号：没有英文候选时 AI 落第二。多于一个的云 / AI 候选按来源分组整组落座而不是丢弃。用户固定了位置（固定位置）的英文候选在排完座位后按固定位置从小到大放回其位置（1 起算，超出列表长度时落到末尾），与来源 `candidate_selection_policy.h` 末尾的固定英文一步相同，因此云 / AI 候选到达不会把它挤到后面；固定位置的中文候选仍随所在分组落座，与来源一致。日文罗马字下读音是单个完整假名时，本地前缀扩为两座，平假名与片假名（か / カ）保持在云候选之前，与来源 `event_listener.cpp` 的 `preserve_single_kana_pair` 一致。运行时用例是 `fixed_english_candidate_keeps_its_seat_when_online_candidates_arrive` 与 `japanese_single_kana_pair_stays_ahead_of_the_cloud_candidate`。
- **置顶或学到顶的英文词占首位**：来源 `NormalizeMixedCandidateOrder` 在排完座位后，把权重是整个混输列表唯一最大值、且没有固定位置的英文候选移到第一位；置顶（或调频学到第一位）写入的权重是整个混输列表（中文候选在内）的最大权重 + 1000，因此置顶后空格上屏的是英文词。锁定 Engine 由 overlay `scripts/apply_engine_promoted_english_first.py` 承接：混输下英文调频按用户看到的完整混输列表排名（专用英文与临时英文仍只在英文词之间排名，临时英文列表首位是原文，不算排名邻居），`CandidateQueries::mixed` 末尾按同一条唯一最大值规则把英文词移到首位，有固定位置的英文词随后由 `apply_candidate_positions` 放回其位置。快照不带权重，运行时 `normalize_online_slots` 把「本地候选存在时 Engine 把英文放在第一位」视为这次提升，保留该英文在首位且不再重复填英文座位，排成 [英文, 中文, 云, AI, …]，与来源 `PromotedEnglishCandidateCanBecomeTheFirstMixedCandidate` 一致。随包词库里英文权重（0–10）远低于中文，没有置顶或学习时顺序不变。Engine 用例在 `test_english_input_session.cpp`，运行时用例是 `promoted_english_candidate_keeps_the_first_seat_with_cloud_and_ai`。
- **以词定字**：`[` 上屏高亮候选的首个汉字、`]` 上屏末字，覆盖三字候选、组合被消耗、无汉字候选与越界索引。高亮候选里没有汉字（英文单词、数字、emoji、颜文字）时 Engine 不接管该键，macOS 宿主与来源 `KeyHandler.cpp` 对 Normal 回复的处理一样，上屏候选词再接该键的标点：GitHub+`[` 上屏「GitHub【」，`-` 按原样追加（GitHub-），其余键走标点表，与 Linux 回落到 `msime_client_punctuation` 相同；候选身份过期或 Engine 调用失败时按键仍被消费，不产出标点。繁体输出下 macOS 先把整条候选词经 OpenCC s2t 转换，再从转换结果取首 / 末汉字，与来源 `ExtractHanCharacter(CandidateTextForOutput(word))` 相同：头发+`]` 上屏「髮」、皇后+`]` 上屏「后」、干杯+`[` 上屏「乾」、面条+`[` 上屏「麪」，与候选窗显示的词一致；取出的字不再逐字转换第二次。汉字范围沿用来源 `IsHanCodePoint`（U+3007、3400–4DBF、4E00–9FFF、F900–FAFF、20000–2FA1F、30000–323AF），日语方案与 Unicode 模式不转换。由 `shortcut` 与 `chinese-text-conversion` 原生测试钉住。
...
- **联网释义的中文候选范围**：与来源 `translation_gloss.cpp` 的 `IsCloudTranslatableChinese` 一致，共享 `crates/client-core/src/translation.rs` 的 `is_cloud_translatable_chinese` 只要求候选里至少有一个汉字（含扩展 A、扩展 B 至 E（U+20000–2CEAF）与兼容表意文字 U+F900–FAFF），且不含 emoji 与图形符号（U+2600–27BF、U+1F000–1FAFF、区域指示符、ZWJ、变体选择符与键帽组合符）；ASCII 字母与数字不排除。T恤、卡拉OK、B超、3D打印这类混写词因此会请求在线释义，并按上一条写入用户释义库、之后离线命中；纯 ASCII 与含 emoji 的候选不上网。翻译计划、释义入库、学习入库、离线学习查询与 `online_gloss` 标志都走这一个判断。`filters_cloud_translation_candidates_by_script`（client-core）与 `mixed_script_chinese_candidates_are_planned_and_saved_like_the_windows_source`、`translation_queries_only_clear_chinese_candidates_for_the_network`（host-api）钉住这些规则。
...
两条都落在共享运行时（本仓的 Server 对应物），因此打开 `phrase_preedit` 的宿主自动拥有它们，目前是 macOS、Linux（IBus 与 Fcitx5）与 HarmonyOS。来源另外要求客户端协商过 `CompositionRestore` 且非 UILess，因为它的 TSF 侧要靠一条回复重建组字；本仓这个条件就是 `phrase_preedit` 本身——宿主只有会画 `view.phrase_prefix` 才会打开它，而视图正是每个宿主得知组字变了的唯一途径。

Ctrl+退格把读音删空时，已选的那一段仍留在组字里，对应来源的 `keep_creating_word_after_empty_raw`（`server/src/ipc/event_listener.cpp`）。此时退格把最后一次选词吃掉的读音还回来，再按 Ctrl+退格删掉最后一段已选的词，回车上屏、Esc 丢弃、继续打字母接着组这个词。没有可退回的选词时，普通退格删空读音仍把已选的词上屏，与来源一致。宿主因此把非空的 `view.phrase_prefix` 也算作组字：macOS 的 `MSIMEViewHasComposition`、Linux 两套前端共用的 `view_has_composition`（`platforms/linux/src/core/PhrasePreedit.h`）、HarmonyOS 的 `HardwareKeyRouter.composing`。回车、空格、Esc 与标点因此不会在组字仍显示那段词时漏给应用；运行时在只剩已选词时上屏或丢弃它，都把这个键记为已处理。
...
**查过、与来源同源的**：简繁输出。来源用 OpenCC 的 `s2t.json`，macOS 经共享导出 `msime_client_simplified_to_traditional` 使用同一提交的 OpenCC s2t 词级词典（见上文「简繁转换」），不用 `CFStringTransform` 逐字转换；`chinese-text-conversion` 原生测试钉了头发→頭髮等词级难例。
...
- Linux（ICU）仍是逐字转换，要统一只需改调同一个导出，本批不动。
...
- macOS 同样调用 `msime_client_simplified_to_traditional`（`platforms/macos/src/core/ChineseTextConversion.mm`），不用 `CFStringTransform`。
