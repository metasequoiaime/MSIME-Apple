...
- 证据：新增 `voice-cue-player`（`tests/voice/VoiceCuePlayerTest.mm`），在临时 bundle 里验证两段产品音频被找到并能解码、只缺结束音时开始音仍用产品音频而结束音回落、没有资源的主 bundle 下初始化不失败；`bundle-contents` 增加对 `Resources/audios/start.mp3` / `end.mp3` 的检查，并实测删掉构建产物中的 `end.mp3` 后该项失败。未在已安装的输入法里实际听过提示音，也未验证录音期间开启「静音其他音频」时提示音能否听到（当前静音实现改的是输出设备，会连同提示音一起静音，留给后续静音工作处理）。（2026-09-23 更新：静音改为等开始提示音播完再生效，见下一节「macOS 录音静音不再吞掉提示音，并跟随默认输出设备」。）

### macOS 录音静音不再吞掉提示音，并跟随默认输出设备（2026-09-23）

- 缺口：来源 `voice_input_service.cpp` 的 `StartRecording` 先 `play_start()` 再 `MuteSystemAudioIfEnabled()`，结束与取消时先 `RestoreSystemAudioIfMuted()` 再 `play_end()`；`system_audio_muter.cpp` 只静音其他进程的音频会话，并通过 `OnSessionCreated` 把录音中新出现的会话也静音，所以两段提示音都听得见。macOS 的 `VoiceAudioMuter.mm` 静音的是整个默认输出设备（`kAudioDevicePropertyMute`），而 `InputController.mm` 在开始录音前就静音、开始提示音到 `voiceCaptureDidStart` 才播，开启「录音时静音其他声音」时开始提示音根本听不到；录音途中默认输出设备换了（插拔耳机、连上 AirPods），新设备照常出声。
- 取舍：macOS 13（本仓部署目标）没有静音其他进程的公开接口。14.2 起的 CoreAudio process tap（`CATapDescription` 的 `CATapMuted`）能做到，但要求部署目标抬到 14.2，并让输入法申请「系统录音」权限，为一个静音选项换来一次录屏级的权限弹窗不划算，所以仍按设备静音，改在时序上对齐来源。
- 做法：静音从开始录音的入口挪到 `voiceCaptureDidStart`。开启开始提示音时，`MSIMEVoiceCuePlayer` 新增的 `playStartCueThen:` 在 `NSSound` 报告播完（`sound:didFinishPlaying:` 为 YES）后执行 `MSIMEVoiceAudioMuter` 的 `deferredMute`；`NSSound` 若一直不报告，按音频时长加 0.5 秒兜底执行；播放失败时立即执行；重播会丢弃尚未执行的那一次。关闭开始提示音时开始录音即静音。`deferredMute` 在其后任何一次 `restore` 后失效，录音在提示音播完前结束（短按、取消、失败）不会再去静音。结束顺序原本就是先 `restore` 再播结束提示音，与来源一致，这次用例钉住。
- 跟随设备：静音生效后在系统对象上监听 `kAudioHardwarePropertyDefaultOutputDevice`（回调投递到主队列），默认输出变了就先把此前静音的设备恢复原状，再静音新的默认设备；新设备若已被用户静音则不接管，事后也不去解除；重复通知同一台设备不会造成一次解除再静音的闪断；`restore` 时注销监听。只有本对象改过静音状态的设备才会被恢复。
- 崩溃恢复：原来的恢复记录只能记一台设备。默认输出因为原设备被拔掉而切换时，原设备暂时无法恢复，又必须静音新设备，所以记录可以同时欠多台（上限 8 台）：只欠一台时仍写原来的 `version 1` 格式，旧版本也读得懂，多台时写 `{"version":2,"uids":[...]}`。仍找不到的设备继续留在记录里，等它重新出现时恢复，但不再挡住下一次录音静音当前的默认设备（此前会一直失败到那台设备接回来）。
- 证据：`voice-audio-muter` 增加录音中切换默认设备、切回、重复通知、新设备已被用户静音、原设备消失后切换及其后续录音与恢复、监听注销后迟到的回调、不提供监听函数、`deferredMute` 被 `restore` 作废，以及多设备记录写入与逐台收缩；`voice-mute-recovery` 增加 `version 2` 记录的恢复与几种非法形式；`voice-cue-player` 用 0 音量实际播放，验证完成回调在提示音结束后只执行一次、重播丢弃前一次回调；`http-voice-controller` 用真实 `MSIMEVoiceAudioMuter` 加合成设备核对「开始提示音 → 静音 → 解除静音 → 结束提示音」的顺序、提示音播完前取消不静音、启动失败不静音、关闭开始提示音时立即静音。未在已安装的输入法里实际听过，也未用真实耳机或 AirPods 做切换验收。

### macOS 录音不再在 60 秒处作废（2026-09-23）

- 缺口：来源 `voice_input_service.cpp` 的 `StopRecording` 总是把录到的音频交出去；批量识别在 `voice_batch_protocol.cpp` 里只受 `upload_sample_limit = (maximum_encoded_audio_bytes - 44) / 2` 约束，即 20 MiB 的 16 位 WAV（16 kHz 下约 655 秒），豆包流式的 `doubao_asr_client.cpp` 则不设上限，WinHTTP 只给单次网络操作设超时。macOS 却在三处写死了 60 秒：`VoicePCMBuffer.mm` 超过 60 秒就让整段录音失败，界面报「录音失败」（错误文案是「录音格式无效或超过 60 秒限制」），`HTTPVoiceRequest.mm` 拒收超过 60 秒的 PCM，`DoubaoVoiceRequest.mm` 拒收超过 60 秒的样本；豆包连接另有按 60 秒录音估出来的 100 秒整场时限（`timeoutIntervalForResource = 100` 加一个 100 秒的兜底失败），说到第 100 秒左右会被直接掐断。
- 批量：上限按实际上传的格式算。macOS 采集的是 float，但批量服务商收到的始终是共享层编出的 16 位 WAV，所以 `shared/voice/VoiceProviders.h` 新增 `batch_upload_sample_limit`（与来源同一个数，`VoiceProviders.cpp` 用 `static_assert` 对 Engine 常量核对，原先那里的局部常量改用它），再减去 SiliconFlow 两端各 0.2 秒的补静音得到 `batch_capture_sample_limit`（约 654.96 秒），保证录满的一段对每家批量服务商都编得出来——来源在这 0.4 秒的窗口里对 SiliconFlow 会编码失败，这里不复制这一点。`MSIMEVoicePCMBuffer` 默认按这个数封顶，到顶后不再收音频、也不报错，采集照常进行，等用户照常松开再把录到的部分提交；内存因此有界（float 约 40 MiB）。来源在这种情况下是先把全部音频攒在内存里，提交时报「录音数据无效或超过 20 MiB 上传限制」并丢掉整段；这里改为截断提交，属于有意的差异：说了十几分钟的用户至少拿到前 11 分钟的文字，而不是一个字都没有；该差异只在超过约 11 分钟时出现。`HTTPVoiceRequest` 不再按 60 秒拒收，而是按服务商截到可提交的长度：云端用上面的上限，本机 Whisper 用 Engine `WhisperWorker` 自己校验的 60 秒（`local_asr_sample_limit`，来源没有本机识别）。（2026-09-23 更新：「采集照常进行，等用户照常松开」不再成立——录满服务商能收的长度后宿主会像松开一样自动结束录音，见「macOS 录音录满上限时自动结束，不再无声截断」。）
- 流式：豆包取消长度上限和 100 秒整场时限，保留 30 秒的请求超时和结束后等最终结果的 30 秒时限。流式录音用不封顶的 `MSIMEVoicePCMBuffer`，每次交出的样本随即释放，缓冲里只剩尚未发送的部分，录多久内存都不增长；`finishWithError:` 相应改为返回尚未交出的样本（从不 drain 的批量录音即完整录音），`finishPCMStreamingWithError:` 的行为不变。
- 证据：`voice-pcm-buffer` 验证默认上限下录满 657 秒得到恰好 `batch_capture_sample_limit` 个样本且加补静音后不超过 20 MiB、小上限落在一个缓冲中间时按样本截断且截下的是录音而不是转换器填充、到顶后继续 append 不失败；`voice-pcm-capture` 把原来「60.1 秒触发失败并停止采集」改为不失败、不停止、完整提交；`voice-pcm-stream` 验证不封顶的流在超过批量上限后仍逐样本交出，finish 只返回未交出的尾部；`http-voice-request` 通过回环 fixture 实际上传超过上限的录音，确认 WAV 数据正好是上限且不超过 20 MiB；`doubao-voice-request` 通过回环 WebSocket 发送 61 秒并确认服务端收齐 976000 个样本。未在已安装的输入法里实际录过 60 秒以上，也未对真实服务商上传接近 20 MiB 的音频；100 秒整场时限的移除没有自动化用例覆盖（需要真实等满 100 秒）。本仓 `platforms/windows/src/voice/VoiceInputSession.cpp` 自己的批量路径仍有 60 秒上限（`kMaximumSamples`），不在这次 macOS 改动范围内。

### macOS 语音识别失败时显示服务商给出的原因，并提示未填写 ASR Token（2026-09-23）

- 缺口：来源 `voice_input_service.cpp` 的 `Recognize` 在 HTTP 非 2xx 时用 `JsonErrorMessage` 取服务商 JSON 里的 `error` / `error.message` / `message`（附 `code` 与 `data`），取不到就显示 `HTTP N`，前缀「语音识别失败：」；SiliconFlow 5xx 再补一句服务端内部错误的说明和 `x-siliconcloud-trace-id` 追踪 ID；网络失败显示「语音识别请求失败：」加原因。`doubao_asr_client.cpp` 分别报连接失败（按旧版 App ID/Access Token 或新版 API Key 提示该检查哪项）、握手失败和带 `code` 的服务端错误。未填 Token 时来源直接提示去设置里填写。macOS 却只有固定类别文案：共享层把状态码写进 `what()`（「Voice HTTP status N」）后被 `HTTPVoiceRequest.mm` 吞掉，豆包所有失败都走同一个 `DoubaoFailure`，浮层只显示「语音识别失败」一类的话；没有 Token 时照常开始录音，说完才失败。
- 做法：`shared/voice/VoiceProviders.h` 新增 `CloudAsrError`（`what()` 保持原来的英文诊断，Windows 宿主 `VoiceInputSession.cpp` 的行为不变）及三个纯函数 `cloud_asr_error_detail` / `cloud_asr_status_message` / `cloud_asr_transport_message`，文案逐字取自来源，响应体兜底截到 240 字节且不切断 UTF-8 字符；追踪 ID 由 curl 头回调按每次尝试读取，最长 128 字符。macOS 请求把这句话放进 `NSError` 的 `NSLocalizedFailureReasonErrorKey`，`NSLocalizedDescriptionKey` 不变；新的 `VoiceFailureMessages.h` 只读取本宿主语音域的这个键（系统错误的 failure reason 是写给开发者的，不显示）。豆包按 socket 是否真正打开（收到过消息或握手响应为 101）区分连接失败与握手失败，认证模式按是否带 `x-api-app-key` 判定。浮层新增 `showFailure:detail:`，状态行仍是类别文案，原因显示在正文区域。未填 Token 且没有共享 provider socket 时，`toggleVoiceInput:` 在请求权限、开始采集之前就在浮层报「请先在设置的“语音输入”分区填写当前 ASR 提供商的 API Token。」，不弹模态窗口。原因里不含 API Key、请求头或请求体。
- 证据：新增 `voice-failure-messages`（`tests/voice/VoiceFailureMessagesTest.mm`）覆盖各种 JSON 形态、240 字节截断落在多字节字符上、SiliconFlow 5xx 与追踪 ID、传输失败、豆包三类文案与两种认证提示、只读本宿主语音域；`http-voice-request` 用回环 fixture 返回 401 JSON 与带追踪 ID 的 500（SiliconFlow 重试两次），核对原因文字、描述不变且不含 Token；`doubao-voice-request` 逐路径核对原因（服务端 code、重定向后的 API Key 连接提示、拒绝连接时的 App ID 连接提示）；`voice-provider-options`、`voice-wave-overlay` 与三个控制器用例覆盖 Token 判定、浮层正文和未填 Token 时不开始采集。完整 ctest 132/132 通过。未对真实服务商或真实豆包凭据验证（真实端点上 `response` 是否报告 101 未实测），未在已安装的输入法里看过浮层效果；走共享 provider socket 的路径仍是类别文案；豆包发送音频失败与超时仍是通用文案，因为来源对这两种情况也不给具体原因。

### macOS 语音浮层改用来源的 12 条波形、胶囊布局与圆形操作按钮（2026-09-23）

- 缺口：来源 `server/src/voice-input/wave_overlay.cpp` 按状态切换四种尺寸——录音时 78x32 的紧凑波形，识别/润色时 112x40 的「识别中...」/「处理中...」胶囊，锁定录音或等待结果时 142x40、两端各一个圆形「×」/「✓」按钮的操作条，流式转写时 420x112 的转写面板（最多 3 行，放不下时丢掉最早的文字并以「…」开头）；12 条波形每 16 ms 按多谐波运动刷新，浮层位于工作区底部上方 10 px。macOS 的 `VoiceWaveOverlay.mm` 只画一根 6 pt 宽的竖条和「正在录音…/正在识别…/正在润色…」文字，只要设置了动作回调就显示「取消」/「确认」两个文字按钮，尺寸是 156x44 或 380x180 的可滚动文本框，底边距 32 pt。
- 做法：尺寸按点照搬来源，布局优先级（操作 > 状态文字 > 转写 > 紧凑）、配色、圆角、透明度、字号与行高（15 pt / 22 pt，状态 14 pt）同来源。波形由 `NSTimer` 以 16 ms 驱动，只在浮层可见且显示波形时运行，隐藏、切到状态文字或失败提示时停止并清零。圆形按钮由波形视图绘制，点击仍由透明 `NSButton` 接收并保留「取消」/「确认」无障碍标签；显示规则同来源：按住录音时不显示，按住期间按 Space 锁定后（`MSIMEVoiceHoldShortcut::locked()` → `setRecordingLocked:`）或识别/润色进行中才显示，只有操作条可见时浮层才接收鼠标。转写不再是可滚动文本框，而是按 `NSLayoutManager` 实测行数二分出能放进 3 行的最新文字，切点落在组合字符序列边界上（来源只避开 UTF-16 代理对）。纯函数 `MSIMEVoiceWaveOverlayLayoutFor` / `MSIMEVoiceWaveOverlaySizeForLayout` / `MSIMEVoiceWaveAdvanceLevels` / `MSIMEVoiceTranscriptVisibleText` 放在 `VoiceWaveOverlay.h`。主题解析（`voice_theme` 覆盖全局 `theme`）不变。
- 与来源的差异：来源的失败提示是模态消息框，这里保留上一节的浮层失败提示——类别文案占状态行，服务商原因占转写区（同样截成 3 行），宽度随文案加宽；锁定且有转写时来源的操作条仍按转写把波形上移并把文字画出窗口，这里操作条内不画转写、波形居中；来源流式非上屏路径停止录音后在转写面板里显示「正在识别…」且不带按钮，macOS 三条路径停止录音时一律显示「识别中...」胶囊加按钮，这一处没有按路径细分。
- 证据：`voice-wave-overlay` 新增布局优先级与四种尺寸、波形电平（静音保持圆点、说话时 12 条都起伏且中间更高、电平始终在 0…1、停止收音后回落到圆点、同一时刻结果确定）、转写截断（合成行数计数下保留最新文字、代理对不被切开、真实 15 pt 排版下恰好 3 行且再多一个字就超出、尾随换行算一行）的用例，并改为核对紧凑/锁定/识别/转写/失败各状态的尺寸、按钮可见性与无障碍标签、动画计时器随隐藏停止、底边距 10 pt；`voice-hold-shortcut` 核对 Space 锁定状态；`live-voice-controller` 核对按住后按 Space 会通知浮层锁定。完整 ctest 132/132 通过。浮层截图只在测试进程里离屏渲染核对过，未在已安装的输入法里实际录音查看，也未在多显示器或 Retina/非 Retina 切换下验收。

### macOS 录音录满上限时自动结束，不再无声截断（2026-09-23）

- 缺口：上一节让批量录音到上限（云端约 655 秒、本机 Whisper 60 秒）后截断提交，但截断对用户不可见：`VoicePCMBuffer` 到顶后丢掉后续音频仍报成功，采集继续，浮层波形还按实时输入电平起伏，本机 Whisper 则在 `HTTPVoiceRequest` 提交时静默截到 60 秒。用户以为还在录，松开后只拿到前一段文字，没有任何提示。来源保留全部音频，提交时由 `Recognize()` 返回「录音数据无效或超过 20 MiB 上传限制。」并用消息框告诉用户原因，丢失不会是无声的。上一节把这一差异写成「有意的差异」，但漏了「用户看不到」这一点。
- 做法：`MSIMEHTTPVoiceRequest` 新增只读的 `sampleLimit`（本机 Whisper 为 `local_asr_sample_limit`，其余为 `batch_capture_sample_limit`），`recognizePCM:` 改用它截断。`startHTTPVoiceInputWithOptions:` 的电平回调在主线程上检查已采集时长，录满 `sampleLimit` 就重置按住快捷键状态并调用 `finishHTTPVoiceInput`，与用户松开完全相同：浮层立即转为「识别中...」、播放结束提示音、提交已录的部分；随后的物理松开不会把识别当作第二次停止而取消。截断提交这一差异保留（说十几分钟的用户仍拿到前 11 分钟的文字，而不是整段报错），但现在用户在上限那一刻就能看到录音已结束。另修正 `VoiceCuePlayer.h` 的错误注释：它原先说提示音从输入法进程播放、静音其他音频时仍可听到，实际静音是整台默认输出设备，所以控制器等开始提示音播完才静音、结束时先恢复再播结束提示音。
- 证据：`http-voice-controller` 新增用例：上限设为 60 秒，59.9 秒时的电平回调不结束录音；60 秒时的回调提交一次、浮层进入识别状态、结束提示音播放一次；排在其后的电平回调不会再次停止（否则会取消识别）；识别结果照常上屏。`http-voice-request` 核对云端请求的 `sampleLimit` 等于 `batch_capture_sample_limit`，构建带本机识别时本机请求等于 `local_asr_sample_limit`。完整 ctest 132/132 通过。未在已安装的输入法里实际录满 60 秒或 11 分钟验证，本次构建关闭了本机 Whisper（`MSIME_SHARED_VOICE_LOCAL_WHISPER=OFF`），本机请求的 `sampleLimit` 断言未实际执行。
