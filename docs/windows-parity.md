...
**需要所有者拍板、本仓不单方面动的：** 要动 `engine-lock.json` 的几项（英文词库的「词」与「显示」不能不同、加加辅助码与全拼备选切分等引擎侧能力），因为它们的影响面覆盖全部平台。

**本机拿不到的证据：** Windows 主机上的安装与真实编辑器交互（只有交叉构建、i686 语法与 79 个原生用例三级证据）；Linux 桌面上的目视验收；HarmonyOS 与 iOS 的真机链路（模拟器与 HAP 构建已通）。凡是「未在真实编辑器里验收」的行，写的都是这个意思。
