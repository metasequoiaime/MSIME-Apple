...
- HarmonyOS（成对开启时的第二个引号，本批顺带修）：成对开启时引擎同样轮换引号（这一点 overlay 前后都一样），而 `KeyboardSession.punctuation` 只对以左半边结尾的提交补右半边，没有来源 `KeyHandler.cpp` 里的 ” → “ 改写，于是第一次 `"` 得到 “|”，第二次引擎给出 ”，`PairedPunctuationPolicy.completion` 认不出它，编辑器里落下一个孤立的 ”（“”|” 这样的错对）。现在由 `PairedPunctuationPolicy.reopenQuote` 在 `apply` 上屏之前把 `"` 键结尾的 ” 改回 “、`'` 键结尾的 ’ 改回 ‘，再照常补右半边；成对关闭时不改写，交给引擎轮换。`keyboard-logic.test.ts` 新增一组断言（含带组字前缀的提交、只改写按下的那个引号键、关闭时原样），把改写换成空操作后该组红在第一条上。仍缺两项，未在本批补：HarmonyOS 没有「跨越式闭合」（光标右侧正是刚补的右半边时再按右符号应跨过去而不是再插一个，Windows、macOS、Linux 都有），也没有按宿主排除成对补全；未在真机上验收。
- iOS（只改注释）：iOS 不补右半边，引擎也只提交按键对应的那一个符号，所以没有这个问题；`KeyboardViewController.swift` 里声称「引擎负责成对标点并提交两个符号」的注释是错的，已改正，行为未动。
