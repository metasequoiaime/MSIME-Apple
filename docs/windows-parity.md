...
**Windows 的显示仍是字母，理由具体**：TIP 的组字文本是它自己按收到的按键本地追加的（为了延迟），不是从视图里取的字符串——只有退格那一条分支会用 `result.view.preedit` 整串替换。要让它显示假名，得把每一次按键都改成「用视图里的读音整串替换组字」，那是 TIP 热路径的改动，本机只有交叉构建这一级证据。**但它的回车已经不受影响**：挂着宿主引擎适配器时走的是引擎驱动的提交（同日的另一批），提交的就是假名；剩下的只是就地结束组字那条旧路径。
