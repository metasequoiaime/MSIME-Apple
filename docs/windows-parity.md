...
进一步的设备排查把范围又收窄了一大截，现在的表述是：**2in1 上硬件字母只在屏幕键盘界面打开时才能起一个新的组合串。** 三项互相印证的观察：
...
3. 关闭屏幕键盘界面后再注入同一个字母，诊断日志立即触发，组合串仍为空。

因此缺陷不在按键投递、不在 Engine、也不在宿主侧的方案与模式判断，而在"没有任何界面打开时，首个字母未能让 Engine 起一个组合串"这一处。
