...
剩下的一项是**安装后的交互验收**——不是代码缺口。#3270 让预览版继承一个本次会话开始时就在输入源列表里的标识之后，它不再必然要求先注销登录：从当前 develop 构建安装后实测两个输入源都已注册启用，可以直接从输入菜单选中试打。要注意查询结果在替换 bundle 之后会有分钟级的一段时间读不到、之后自行恢复，按那段时间里的读数下结论会错——测量过程见下面的《安装后验收》一节。
