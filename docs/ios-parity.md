...
1. **「更多」面板的折线取舍**（见上一节）。等你定。
2. **`smart_punctuation_repeat` 与 `smart_punctuation_space_convert` 在 iOS 上没有任何消费方。** macOS（`InputController.mm`）和 HarmonyOS（`SmartPunctuationRepeatPolicy.ts` / `SmartPunctuationSpacePolicy.ts`）都实现了，共享设置页对 iOS 用户照样显示这两个开关，打开之后什么都不会发生。这**不是** MSIME-Apple 的迁移缺口——来源仓库里根本没有智能标点这个功能，它是本仓自己按 Windows 基线加的。所以它不在本轮范围内，单独记在这里：修它要先决定那两条策略是留在各平台各写一份（现在已经两份了），还是像 `punctuation.rs::route` 那样下沉到 `client-core` 再由各宿主消费。按架构该是后者，但那会同时动到 HarmonyOS 和 macOS，是它自己的一次改动。
