...
**MSIME-Apple 的 iOS 功能已经全部迁移完。** 这句话的依据和它管不到的范围都写在下面。

依据是九条机械比对轴，全部在固定基线上跑完，其中后六条又在双方当前 HEAD 上重跑过一遍（见下一节）：

| 轴 | 规模 | 未命中的去向 |
| --- | --- | --- |
| 源文件名 | 192 | 集中在 `shared/apple-bridge/` 与 `shared/backend/`，分别由 `crates/host-api` 的 C ABI 和 `crates/client-core` 取代 |
| 中文文案 | 1666 | 测试断言、措辞差异、Tauri 页以不同表述覆盖、注释里的引号 |
| 符号 | 584 | 61/68 在被取代的 ObjC 桥接层；其余是改名或重构等价物 |
| 可达控件标识 | 254 | 2 个：命名差异与 Tauri 覆盖 |
| 同名文件成员 | 116 对 | 16 个文件有差集，逐个核实全是重构等价物（候选注解并成 `KeyboardCandidateAnnotation`、「更多」面板换成显式页枚举、表情面板换成分页 ABI 等） |
| 测试断言 | 232 | 目标该区域 293 个，整体是超集；按名未命中项逐条对到改名或拆分后的用例 |
| 设置面控件文案 | 219 | 2 条，均已由 Tauri 以不同措辞覆盖 |
| 界面测试 | 38 | 目标 39，组织方式不同、覆盖面相当 |
| iOS 偏好同步字段 | 8 | 全部对上，目标为超集（多出三个调频字段） |

加上按来源「为什么这样做」的注释逐条回验行为——这是唯一能抓到「两边都有、行为不同」那一类的方法，而**三轮里查实的三个真实缺口全都是这样找到的**，不是任何一条存在性比对抓到的：

1. 双拼键面提示是一张会漂移的手写表（小鹤 `K` 少了 `uai`），而来源在头文件里写明前端不能硬编码 keymap（#3329）
2. 日语模式列用的是来源明确换掉的那种环式约束写法（#3373 里同时确认了「更多」面板的折线不是同类问题）
3. 模拟器手写 fallback 只画笔画、不说自己没有识别能力，而来源在同一处给出了那句话（#3377）

三个都已修掉并合并。范围外但顺带补上的一件：`smart_punctuation_repeat` / `smart_punctuation_space_convert` 在 iOS 上曾经没有任何消费方（#3367 / #3372）。

### 这句话管不到什么

**证据等级是第 3 级（模拟器运行），不是第 5 级。** 按 [ARCHITECTURE.md](../ARCHITECTURE.md) 的分级：签名构建与装机做过（#3342，iPhone 17，`devicectl` 安装成功，`nm` 确认改动在产物里），但**启动、键盘扩展启用、真实编辑器打字与生命周期没有做**——`devicectl process launch` 被设备锁屏拒绝，而模拟器上第三方键盘装得上、切不过去（七条办法的实测结论见 `platforms/ios/README.md`）。`KeyboardExtensionEditorUITests` 就是为这一步准备的，键盘一旦在设置里启用它会自己跑起来。

所以「迁移完」说的是：来源的功能在这边都有归宿，而且有强制检查看着；不是「已在真机上验收过」。这两件事本文档一直分开写，这里也不合并。
...
| 同名文件成员 | 116 对，16 个文件有差集，与第二轮结论一致：全是重构等价物，另有两个来源独有文件（`EmojiCatalog` 换成走 ABI 分页的 `KeyboardEmojiCatalog`、`EnglishMixedCandidatesPreference` 换成一次性迁移） |
| 测试断言 | 来源 232、目标 293。按名未命中 28 个（第二轮是 33），逐条对到改名或拆分后的用例；差额来自本轮之前补进去的那几条 |
