...
`engine-lock.json` 把 Engine 钉到具体 commit 的 tar.gz 归档（带 sha256），而不是 submodule；`patches` 为空，改动全部走 13 个 `scripts/apply_engine_*.py` overlay 脚本和 2 个 overlay asset，这样上游 bump 时冲突面清晰可见。四个嵌套依赖（Google-PinyinIME-Rev、utfcpp、miniaudio、whisper.cpp）各自带归档和摘要。
...
**引擎改动走 overlay 脚本，不走 patch。** `engine-lock.json` 的 `patches` 是空的，13 个 `scripts/apply_engine_*.py` 在取回归档后按顺序改写源码。相比一堆 diff，脚本能表达「在这个函数里找到这一段再改」这种带上下文的合并，上游 bump 时要么干净应用要么明确报错，而不是产生一堆需要手工消解的 reject。引擎本身用归档而非 submodule 固定，因此嵌套依赖的 bump 会以裸路径的形式出现在 diff 里，走人工 review 而不是自动合并。
