...
以上全部处在 [ARCHITECTURE.md](../ARCHITECTURE.md) 证据分级的第 1–2 级：源码、单元测试、`verify-local.sh --quick`、设置包构建。**没有任何一项在 HarmonyOS 真机或模拟器上运行过。**

因此下列内容一律没有证据，真机验收时应优先核对：

- NAPI 边界本身。本轮新增七个 C ABI 入口及其 NAPI 包装，C 侧有测试，ArkTS 到 C 那一跳没有。
- 账号、社区、AI 服务的真实往返。
