...
- 目标：本文前三轮的历史结论仍固定在 `develop` 提交 `8011f6dda`；延续的来源测试断言审计记录到本分支 `c4de95cb5`。两者分开写，避免把尚未合入 `develop` 的证据倒灌成已发布状态。
...
延续审计在独立 worktree 中又落了四个尚未合入 `develop` 的切片：`bbba58c92`（云快照原生完整校验）、`3a1a512bc`（云快照请求持久化）、`44e3fbbba`（个人词库按 Engine 规范化并支持多行快捷短语）与 `c4de95cb5`（常驻键盘按 4 条一批持续排空个人词库）。这些只具有源码、单元测试与本地构建证据，不继承下面 2026-09-21 那轮模拟器结论。
