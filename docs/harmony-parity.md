...
截至 2026-09-22：293 个宿主静态方法，11 条平台差异，**0 条待接**。该数字由当前
`origin/develop` 上的 `scripts/test-harmony-unwired-symbols.py` 输出核对。
