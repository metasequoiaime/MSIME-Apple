#!/usr/bin/env python3
"""Reject a vendored validator that differs from the reviewed Engine pin."""
from pathlib import Path
root = Path(__file__).resolve().parents[1]
source = root / 'vendor/MetasequoiaImeEngine/contracts/dictionary/product.py'
# An engine archive that was never unpacked is not a validator that drifted. Reporting both as
# "differs" sends whoever reads it looking for a content change that is not there.
if not source.is_file():
    raise SystemExit(f'{source.relative_to(root)} is missing; run python3 scripts/fetch_engine.py')
