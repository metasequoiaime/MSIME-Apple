...
macOS InputMethodKit 控制器与 iOS 键盘扩展负责创建资源路径和配置、调用本适配器、处理返回的 handled/commit/view，以及系统文本提交。macOS 目标由 `platforms/macos/CMakeLists.txt` 构建，iOS 键盘扩展由 `platforms/ios/project.yml` 注册为 `com.apple.keyboard-service`；本目录只实现共享原生消费边界，不替代系统安装、签名和真机验收。
