...
{
    if (error)
        *error = [NSError errorWithDomain:@"app.msime.ios.services"
                                     code:1
                                 userInfo:@{NSLocalizedDescriptionKey : @"服务数据格式不正确，请检查模型和服务接口。"}];
}
std::string UTF8(NSString *text)
{
    return text.UTF8String ? text.UTF8String : "";
}
} // namespace
...
+ (NSDictionary<NSString *, id> *)transcriptionBody:(NSData *)wav model:(NSString *)model error:(NSError **)error
{
    try
    {
        ...
        return @{
            @"body" : [NSData dataWithBytes:request.body.data() length:request.body.size()],
