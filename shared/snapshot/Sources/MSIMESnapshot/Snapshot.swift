...
struct BackendSnapshotEnvelope: Sendable {
  ...
  static func inspect(_ file: URL) throws -> Self {
    ...
    var finished = false
    var checksum: String?
    let bad = SnapshotFailure(status: 400)
...
private struct SnapshotJSONSyntax {
  ...
  private let bad = SnapshotFailure(status: 400)
...
final class BackendPreparedSnapshot: @unchecked Sendable {
  ...
  init(copying source: URL) throws {
    guard source.isFileURL else { throw SnapshotFailure(status: 400) }
    ...
    do {
      ...
      while let data = try input.read(upToCount: 65536), !data.isEmpty {
        try Task.checkCancellation()
        count += data.count
        guard count <= 512 * 1024 * 1024 else { throw SnapshotFailure(status: 400) }
  ...
  private func readLine() throws -> Data? {
    ...
    while true {
      ...
      if offset == chunk.count {
        ...
        guard bytes <= 512 * 1024 * 1024 else { throw SnapshotFailure(status: 400) }
      ...
      if let newline = chunk[offset...].firstIndex(of: 10) {
        ...
        guard !line.isEmpty, line.count < 65536 else { throw SnapshotFailure(status: 400) }
        return line
      }
      line.append(chunk[offset...]); offset = chunk.count
      guard line.count < 65536 else { throw SnapshotFailure(status: 400) }
    }
  }
}
